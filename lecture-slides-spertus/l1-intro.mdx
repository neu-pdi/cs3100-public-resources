...
<p style={{fontSize: '1.1em'}}>
  The goal: help you better understand how to <strong>design and implement programs</strong>
</p>
<p style={{marginTop: '1em', fontSize: '0.95em'}}>
...
**Students should know from CS 2100:** Requirements, design, implementation (medium-scale)

**What's new here:** Expand scale + explore validation & operations (deeper in CS 4530)

**Key insight:** Good process isn't bureaucracy—it's accumulated wisdom about what works

→ **Transition:** Here's the process we'll follow...
</aside>
...
→ **Transition:** How does Java deliver on its "write once, run anywhere" promise?
</aside>
...
<table style={{margin: '0 auto', fontSize: '0.9em'}}>
  <thead>
    <tr>
      <th style={{padding: '0.5em 1em'}}>Interpreted</th>
      <th style={{padding: '0.5em 1em'}}>Native Compiled</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td style={{padding: '0.5em 1em'}}>Reads & executes line by line</td>
      <td style={{padding: '0.5em 1em'}}>Already in machine code</td>
    </tr>
    <tr>
      <td style={{padding: '0.5em 1em'}}>100s of instructions per statement</td>
      <td style={{padding: '0.5em 1em'}}>Direct CPU execution</td>
    </tr>
    <tr>
      <td style={{padding: '0.5em 1em', color: '#c00'}}>Slower</td>
      <td style={{padding: '0.5em 1em', color: '#060'}}>Faster</td>
    </tr>
  </tbody>
</table>
...
**Example:** Simple operation that's 10 native instructions might be 1000 interpreted

**Where it matters:** Tight loops, computation-heavy code

→ **Transition:** Compilers can also do more sophisticated optimizations...
</aside>
...
## Compilers Can Eliminate Unnecessary Work
...
- Loop optimizations (improve memory access patterns)
...
## Packages Organize Code Like Python Modules
...
**Convention:** Reverse domain name
- Own example.com → com.example.*
- GitHub user octocat → io.github.octocat.*

**Why:** Prevents naming collisions (two orgs can both have "utils")

**Note:** Can choose any name, but convention = recognizable + professional

→ **Transition:** Imports work similarly...
</aside>
...
<aside class="notes">
**Import = make classes available without full qualification**
- Without import: write full path every time
- With import: just use class name

**Special case:** `java.lang.*` auto-imported (String, System, Object, etc.)

**Everything else:** Must explicitly import

→ **Transition:** Three types of comments...
</aside>
...
**Javadoc:** Before classes, methods, fields you want documented. More detail later.
...
<p style={{fontSize: '1.3em', color: '#c00'}}>
  Integers have <strong>fixed sizes!</strong>
</p>
<p style={{marginTop: '1em'}}>
...
**Example:** Integer.MAX_VALUE + 1 = large NEGATIVE number (not 2 billion + 1)

**If you need arbitrary precision:** Use BigInteger (class, not primitive)

**For most purposes:** int is fine, but be aware of limits

→ **Transition:** Quick math to remember sizes...
</aside>
...
- 8 bytes = 64 bits ≈ 18 quintillion values

**Useful trick:** 2^10 ≈ 1000, so 2^32 ≈ 4 × 10^9

**Example:** Store everyone's age on Earth? 8 billion × 1 byte = 8 GB (tractable!)

→ **Transition:** Type choice matters at scale...
</aside>
...
## Choosing the Right Type Saves Memory at Scale
...
<p style={{marginTop: '1em', fontSize: '0.85em', color: '#666'}}>
  Memory was expensive in the 1990s!<br/>
  Today: matters for big data (numpy, pandas are C extensions)
</p>

<aside class="notes">
**Concrete example (storing ages for 8 billion people):**
- byte (1 byte) → 8 GB
- long (8 bytes) → 64 GB

**Historical note:** 1990s memory was precious. Today often doesn't matter... until big data.

**Connection:** numpy/pandas = C extensions using fixed-size types for performance. When scale matters, type size matters.
