...
flowchart LR
    A["1. Accept<br/>Submission"] --> B["2. Copy Student<br/>Files"]
    B --> C["3. Build<br/>Project"]
    C --> D["4. Run Instructor<br/>Tests"]
    D --> E["5. Run Mutation<br/>Analysis"]
    E --> F["6. Report<br/>Results"]
...
* Who can see the test code?
* How do instructors configure grading?
* What happens when 200 students submit at once?
...
## Why Is "Where Tests Run" Expensive to Change?

<p style={{fontSize: '0.85em'}}>
Switching from GitHub Actions to your own servers sounds like one decision — but it touches everything:
</p>

<div style={{fontSize: '0.72em', marginTop: '0.75em'}}>

| What must change | Why |
|-----------------|-----|
| **Packaging & deployment** | GitHub Actions use their own syntax and semantics |
| **Authentication & repo access** | GitHub Actions gets built-in access to the repo; your own server needs separate credentials and permissions |
| **Scaling** | GitHub Actions scales automatically; a custom server needs a job queue, worker pool, and ops team |

</div>

<div className="fragment">
<p style={{fontSize: '0.82em', marginTop: '0.75em', color: '#9370DB', fontWeight: 'bold'}}>
No single module "owns" the decision — but all of them depend on it. That's what makes it architectural.
</p>
</div>

<aside className="notes">
**Key point to emphasize:**
- Students may initially think "it's just one config change" — the point is that the execution environment is a foundational assumption baked into many other pieces
- The action's packaging format, its trigger mechanism, its credential model, its scaling model — all of these were designed around GitHub Actions
- Reversing it means revisiting every one of those pieces, not just editing a single file

**Analogy if helpful:**
- Switching from municipal water to a private well isn't just replacing one pipe — it means a pump, a pressure tank, filtration, maintenance, and your own monitoring. The "where does water come from" decision is woven into everything downstream.

**Bridge:**
- This is why we say architecture = decisions that are expensive to change
- The *cost* comes from how many other decisions were made assuming this one was already settled
</aside>
...
|----------|-------------|------------|
| **Where does grading run?** | GitHub Actions (rented) | Custom job queue on local server |
| **Where does business logic live?** | "Grading action" component — knows Java, Gradle, checkstyle etc. Parses, scores, normalizes | Per-language `Grader` subclasses running locally |
| **How do instructors configure?** | `pawtograder.yml` configuration in each assignment repo | Web UI forms |
...
<p style={{fontSize: '0.9em', marginTop: '0.5em'}}>
Neither architecture is "wrong" — they likely reflect <strong>different requirements in different contexts</strong>.
</p>
...
| **Infrastructure** | Owned servers, full control | Rented (GitHub Actions), zero operational responsibility |
...
<div style={{display: 'flex', flexDirection: 'column', gap: '0.75em', fontSize: '0.65em', marginTop: '0.5em', width: '100%'}}>

<div style={{padding: '0.6em', border: '2px solid #4CAF50', borderRadius: '8px', width: '100%'}}>
...
<div style={{padding: '0.6em', border: '2px solid #FF9800', borderRadius: '8px', width: '100%'}}>
...
**Why course isolation matters — emphasize this!**
- Imagine 10 different courses all using the same grading platform
- If an instructor tweaks their grading config and it breaks ANOTHER course's grading, that's a nightmare
- Worse: if an instructor is mid-semester and can't update their test cases because it requires a platform-wide deployment
- The architecture MUST let each course evolve independently — new assignments, new tests, new grading criteria — without coordinating with other instructors or risking their stability
- This is changeability at organizational scale: "I can ship changes to MY course without a meeting"
...
- L36: How do we deploy changes safely? How do we keep this maintainable over years? (Deployability, Sustainability)
...
## Review: What's a Service?

<div style={{ fontSize: '.8em' }}>
One definition: A cohesive unit of functionality with a well-defined interface

Best practices for scaleable systems:

* Services are interfaces or abstract classes, not concrete classes
* Services support dependency injection
* Services follow the Single Responsibility Principle
* Interfaces are:
   * general enough for multiple implementations
   * specific enough to meet the needs of clients
</div>
...
<div style={{ fontSize: '.8em' }}>
A library system might include a NotificationService:

```mermaid
classDiagram
    class NotificationService {
        <<interface>>
        +notifyHoldReady(Patron, Hold)*
        +notifyHoldExpired(Patron, Hold)*
    }
...
|-----------|---------------------|----------------|
| **Config file: `pawtograder.yml`** | Every assignment (weekly) | Instructor |
...
<Img
  src="/img/lectures/web/l18-actors-v2.jpg"
...
An instructor configuring a YAML file shouldn't need to know about OIDC tokens. The API shouldn't need to know how tests are run. <em>We'll design the better version after finishing the heuristics.</em>
</p>
</div>
...
- `Grader` subclasses per language/check
  - java_style_grader, junit_grader
  - questions_grader
...
- Bottlenose's `Grader.find_grader()` is Service Locator at architecture scale
...
You *could* test a `Grader` subclass by feeding it canned test output, but you'd need to:
...
<div style={{display: 'flex', flexDirection: 'column', gap: '0.5em', fontSize: '0.65em'}}>

<div style={{padding: '0.5em', border: '2px solid #4CAF50', borderRadius: '8px'}}>

**Option A: Logic in the Action**

The action (TypeScript code in the GitHub Action) knows how to build Java (Gradle) and Python. It normalizes results before sending to API.
...
<div className="fragment">
<p style={{marginTop: '0.5em', color: '#9370DB', margin: '0.5em 0 0 0'}}>
Pawtograder chooses <strong>Option A</strong>. The action normalizes everything into a standard `AutograderFeedback` format. The web API remains simple and scalable.
</p>
</div>
</div>
...
<div style={{fontSize: '0.6em', marginTop: '-1em'}}>
...
{/* This doesn't seem to fit with the first half of the slide.
...
<div style={{transform: 'scale(0.75)', transformOrigin: 'top center', marginBottom: '-4em'}}>
```mermaid
graph TD
    subgraph Actors
        Student([Student])
...
<p style={{fontSize: '0.7em', marginTop: '0em'}}>
