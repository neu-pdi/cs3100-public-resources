...
* Complete the [student survey](https://form.jotform.com/260086787355165) by Friday.
...
A specification document as a wide-open barn door with no filter at all. A parade of implementations walks through—some with ✓ checkmarks (good), but also clearly buggy ones with skull icons: 'Returns 0 if item not found!' 'Crashes randomly!' 'Deletes your files!' The bouncer (labeled 'Spec') shrugs helplessly: 'The spec just says return an index, not what to do if item isn't found!' A gauge shows 'Flexibility: ANYTHING GOES'. Yellow caution tape everywhere. Annotation: 'Accepts buggy implementations—undefined behavior lurks.'
...
A perfectly balanced specification shown as a well-designed filter/gate system. The gate is sized exactly right: buggy implementations (with X marks) bounce off, while ALL valid implementations (linear search, binary search, parallel search—all with ✓) pass through smoothly. The spec text is clear and readable: 'Returns AN index containing target, or throws exception if not found.' Developers reading it all have the same thought bubble. A 'Specification Quality' gauge shows three green bars: Restrictive ✓, General ✓, Clear ✓.
...
<Img
    ...
    alt="Concept: 'The Undefined Behavior Roulette Wheel'

Layout (directional words are for positioning only, not text in image):

CENTER: A roulette wheel labeled 'What happens when arr is empty?' with wedges: 'Return 0', 'Return -1', 'Throw Exception', 'Sing Happy Birthday', 'Hang forever'. A gleeful toddler at the controls, happily spinning the wheel.

LEFT: A javadoc showing '@return the sum' with no mention of empty arrays. A detective points at the gap.

RIGHT: Three developers with different implementations (returns 0, throws exception, returns -1), all saying 'My implementation matches the spec!' Other developers below are frustrated: 'I upgraded the library and now my tests fail!'

BOTTOM: A well-specified version with '@return 0 if the array is empty' highlighted in green.

Style: Casino/game-show aesthetic with dramatic lighting."
  />
<aside className="notes">
**The problem:**
- Spec says nothing about empty array input
- Behavior is "undefined" for empty arrays
- Implementation could: return 0, return -1, throw exception, sing Happy Birthday...
- All would technically satisfy this spec!

**Why this matters:**
- Callers can't predict what happens with empty arrays
- Different implementations might behave differently
- Bugs hide in undefined corners

**Real-world impact:**
- Team A writes implementation that returns 0
- Team B writes caller expecting an exception
- Neither is "wrong" according to spec
- Bug discovered in production

**Note on null:** We typically don't document NPEs in Java since they're implied by the language, but empty array behavior is genuinely ambiguous and should always be specified.
...
<ul style={{fontSize: '0.8em'}}>
  ...
  <li>Empty array → returns 0</li>
  <li>Null → throws NullPointerException</li>
</ul>

<aside className="notes">
**What changed:**
- Added `or 0 if arr is empty` to the @return tag
- Now callers know EXACTLY what happens for empty arrays
- No more ambiguity: empty returns 0 (the mathematical identity for addition)

**Note on null:**
- NPEs are often considered implicit in Java (the language throws them for you)
- But we document them explicitly here for completeness
- Empty array behavior is the key ambiguity that MUST be documented
...
<p style={{fontSize: '0.9em'}}>Since `Student` doesn't override `equals()`, it inherits this version—which just checks reference equality.</p>
