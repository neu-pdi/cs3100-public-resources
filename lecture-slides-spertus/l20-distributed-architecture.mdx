...
Network communication doesn't just change HOW components talk — it invalidates assumptions programmers make constantly.

 Students who understand the eight fallacies will make better decisions when they inevitably work on distributed systems. Security isn't a feature you add — it shapes architecture from the start.
...
- Arc 6: Security as an Architectural Concern**
...
<aside className="notes">
**Context:**
Last time we told you: "Distributed Systems Ahead, Here Be Dragons"
...
<aside className="notes">
- T: One more question before looking at some numbers
</aside>
...
→ T: Let's see what can be done with HTTP...
</aside>
...
<strong>REST</strong> (Representational State Transfer) is a set of conventions built on HTTP for structuring APIs.<br/><br/>An HTTP request has: a <strong>method</strong> (verb), a <strong>URL</strong> (resource), and optionally a <strong>body</strong> (data).
</p>
...
<aside className="notes">
- Key REST constraint: each request contains ALL information needed to process it, making horizontal scaling easy
→ REST standardizes how servers communicate success and failure...
</aside>
...
→ Let's generalize this to a pattern...
</aside>
...
<aside className="notes">
→ Even when the network works, it still slows you down...
</aside>
...
→ **Transition:** What do we show the user when it's open?
</aside>
...
→ Let's now take a fresh look at monoliths and microservices
</aside>
...
→ Now let's look again at microservices
</aside>
...
→ There's a particularly bad outcome to watch out for...
</aside>
...
→ What enables horizontal scaling? Statelessness...
</aside>
...
- REST's "stateless" constraint exists specifically for horizontal scalability
...
→ Let's start with a framework for thinking about what we're protecting...
</aside>
...
Confidentiality: Students shouldn't be able to see solutions or other students' grades/submissions

Integrity: Grades and graders shouldn't be tampered with

Availability: We need to keep service up, as much as possible
