...
- The image_url is a reference into object storage (S3, Cloud Storage) — the database holds metadata, not the binary data itself
...
- "The database builds and maintains this index automatically after you create it"

**The write penalty:**
- Indexes aren't free — every INSERT or UPDATE must also update the index
- Read-heavy workloads (like search) benefit enormously
- Write-heavy workloads (like logging) may want fewer indexes

**When to reach for dedicated search:**
- PostgreSQL full-text search is good for moderate scale
- Elasticsearch, Algolia, Meilisearch are purpose-built for search
- They support: fuzzy matching ('distarcted boyfriend' still finds results), ranking by relevance, faceted filters
- Algolia powers the search on many major sites students use daily

**Connection to the poll:**
- "This is infrastructure nobody votes to build — but without it, your search box is broken at scale"

→ **Transition:** Indexes help with search. But what if thousands of users search for the same thing simultaneously?
</aside>
...
search:"distracted" → [4821, 4603, 4512, ...]
...
<aside className="notes">
- Redis is an in-memory key-value store — extremely fast
- The key is typically the query type and string; the value is the result
- Hit rate matters: 99% hit rate means the database sees 1% of the traffic

→ **Transition:** Fast search and retrieval covered. What about sudden traffic spikes?
</aside>
...
<div className="fragment">
<p style={{fontSize: '0.68em', marginTop: '0.5em', color: '#FF9800'}}>
<strong>Common services:</strong> AWS SQS, Google Pub/Sub, RabbitMQ. The queue guarantees every request is eventually processed — even if a worker crashes mid-job, the message goes back on the queue.
</p>
</div>
...
→ **Transition:** We can handle the traffic. But how do users find us in the first place?
</aside>
...
- Managed auth services (Auth0, Supabase Auth, Firebase Auth) handle all of this
- This is a good example of the build vs. buy tradeoff — almost always buy for auth

→ **Transition:** Even with everything running smoothly, things will eventually break...
</aside>
...
- On-call rotations are a real part of engineering jobs — students will encounter this
- Alerting tells you something is wrong; observability tells you why

→ **Transition:** Alerting wakes you up. But then what?
</aside>
...
→ **Transition:** What else might you want?
</aside>
...
- Auth and rate limiting in one place means one place to audit and update
- This is information hiding at the infrastructure level — backend services don't need to know about auth policy

→ **Transition:** We've now covered all the infrastructure behind your meme generator. Let's step back and look at the big picture.
</aside>
...
flowchart TD
    ...
    F --> C[(Cache<br/>Redis)]
    F --> Q[Message Queue<br/>SQS / Pub/Sub]
    Q --> W[Worker<br/>Image Processing]
    W --> S3
    F --> O[Observability<br/>Datadog / Sentry]

    style F fill:#9370DB,stroke:#5b3a8c,color:#fff
```
...
- The same diagram applies whether you're using Lambda, containers, or Edge Functions — the building blocks are the same

→ **Transition:** So what does this mean for how you spend your time?
</aside>
