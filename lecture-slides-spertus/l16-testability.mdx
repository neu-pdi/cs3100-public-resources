...
<p style={{fontSize: '0.8em', marginTop: '0.5em', color: '#9370DB'}}>
  The second version is easier to test: call the method, check the return value.<br/>There's no need to inspect mutated objects or mock repositories.
</p>
...
<div style={{fontSize: '0.8em', marginTop: '.5em'}}>
...
graph LR
    ...
    style Core fill:#e8f4e8,stroke:#4a9,stroke-width:2px
...
<div style={{fontSize: '0.8em', marginTop: '0.5em'}}>
  ...
  <div style={{visibility: 'hidden'}}><strong>Ports:</strong> Interfaces that describe *what* the domain needs, not *how* to get it.<br/><br/></div>
  <div style={{visibility: 'hidden'}}><strong>Adapters:</strong> Implementations of the interfaces to talk to specific technologies</div>
</div>
...
graph LR
    ...
    style Core fill:#f5f5f5,stroke:#ccc
    style Ports fill:#f5f5f5,stroke:#ccc
    style Adapters fill:#f0e8fa,stroke:#96c,stroke-width:2px
```
...
<div style={{flex: 2, fontSize: '.75em'}}>
...
<div style={{flex: 1, fontSize: '.75em'}}>

```mermaid
classDiagram
  direction BT
    class EnergyPricePort {
        <<interface>>
        getCurrentPricePerKWh()* double
    }

    class GridPriceApiAdapter {
        -HttpClient httpClient
        -String apiKey
        +getCurrentPricePerKWh() double
    }

    GridPriceApiAdapter ..|> EnergyPricePort

    style EnergyPricePort fill:#2d4a5a,stroke:#4ac,stroke-width:2px,stroke-dasharray: 5 5
```
...
classDiagram
    ...
    class DeviceControlPort {
        <<interface>>
        setDevicePower(String, int)* void
    }

    class SpyDeviceControlAdapter {
        -Map~String, Integer~ powerLevels
        +setDevicePower(String, int) void
        +getPowerLevel(String) int
    }

    SpyDeviceControlAdapter ..|> DeviceControlPort

    style DeviceControlPort fill:#2d4a5a,stroke:#4ac,stroke-width:2px,stroke-dasharray: 5 5
```
...
<div className="fragment" style={{fontSize: '0.85em', marginTop: '0.5em'}}>
<p style={{color: '#f44336'}}>
  ⚠ Same static call problem — you can't swap in a test double.
</p>
<p>
  But singletons add another issue: <strong>shared mutable state</strong>. One test's data leaks into the next.
</p>
</div>
...
<Img src="/img/lectures/web/l16-xkcd-imposter-syndrome.png"
  width={450}
  alt="Single-panel XKCD cartoon:
