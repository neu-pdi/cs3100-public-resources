...
<aside class="notes">
* Java has changed greatly in its lifetime.
* Backward compatibility was always maintained.
* That means that changes did not break old programs.
</aside>
</Slide>
...
<aside class="notes">
* How could new keywords be added without breaking existing programs?
* They might have used the keywords as variable names.
* The solution is contextual keywords.
</aside>
...
Large code snippet on a note falling to the floor: public void watchBaby() throws CryingBabyException {    baby.putToBed(); // not my problem!}
...
<ul style={{fontSize: '0.8em'}}>
  ...
  <li>Immutability: all fields are <code>final</code>, class is <code>final</code></li>
</ul>
...
<Img src="/img/lectures/web/qr1-stamp.jpg"
  prompt="A single rubber stamp viewed from a three-quarter angle, detailed and realistic with a wooden handle and red rubber base.
The stamp surface shows an overly complex data structure carved into it — a 'Student' object with nested fields visible: 'name', 'email', 'address', 'enrollmentDate', 'GPA', 'courses[]', 'advisor', etc. Far too much information for any single use.
The stamp has just been pressed onto paper, leaving a red ink impression of the entire bloated data structure. A small hand-written arrow points to just the 'email' field with a note: 'Only needed this part.'"
...
* **L**iskov Substitution Principle: Objects of a supertype should be replaceable with objects of its subtypes without breaking the application.
...
<p>`BufferedInputStream` wraps `FileInputStream`, adding buffering without modifying `FileInputStream`.</p>

<p>What is the main advantage of this design over using inheritance (e.g., `class BufferedFileInputStream extends FileInputStream`)?</p>
...
<Img src="/img/lectures/web/qr1-visual-decorator.jpg"
  prompt="A clean, technical illustration showing an exploded isometric view of nested UI components, with each layer floating above the next to show how decorators wrap objects. The only text should be the labels and notes below.
Bottom layer — 'TextView':
A simple text document with readable content, clean and minimal. Label points to it: 'TextView'. This is the core component.
Middle layer — 'ScrollDecorator':
An empty text view with scrollbars on the right side and bottom, with an outer margin. The scrollbars are clearly visible with up/down and left/right arrows. Label: 'ScrollDecorator'. A small note: 'Adds scrolling behavior.'
Top layer — 'BorderDecorator':
An empty text view with a narrow decorative border frame that does not obscure the scrollbars. Label: 'BorderDecorator'. A small note: 'Adds border styling.'

Additional variants floating nearby (smaller scale):
'LineNumberDecorator' — shows line numbers in a gutter on the left
'HighlightDecorator' — shows text with highlighted search terms
'RedactionDecorator' -- blocks parts of the text
Visual style:
Crisp lines in dark blue or black. Each layer has subtle transparency to show nesting. Consistent isometric angle (30 degrees) for all layers."
/>
</Slide>
...
<p className="fragment">With inheritance, we would need a separate class for each combination!</p>
...
## The Four Pillars of OOP
  ...
  <Img src="/img/lectures/web/l7-pillars-of-oop.png"
    prompt="Create an illustration for a computer science lecture slide showing the four pillars of object-oriented programming.

Show a Greek or Roman temple structure with four distinct pillars supporting a roof labeled 'Object-Oriented Programming' or 'OOP'. Each pillar is labeled with one of the four principles:

1. **Encapsulation** - perhaps depicted with a locked box, a wrapped package, or a protective shell icon on or near the pillar
2. **Abstraction** - perhaps depicted with a simplified shape, a blueprint, or a cloud hiding details
3. **Inheritance** - perhaps depicted with a family tree, parent-child figures, or an arrow pointing down through generations
4. **Polymorphism** - perhaps depicted with a shape-shifter, multiple forms, or one interface with many implementations (like one plug fitting different outlets)

The temple should feel sturdy and foundational—these are the principles that support everything else in OOP. The overall mood is educational and approachable, not intimidating.

Clean, modern illustration style with a hint of classical architecture. Horizontal orientation suitable for a presentation slide. No code. Labels should be clear and readable."
  alt="The four pillars of OOP: Abstraction, Encapsulation, Inheritance, and Polymorphism" />
...
<div className="fragment" style={{ marginTop: '2rem' }}>
...
```mermaid
classDiagram
    class AbstractCollectionE["AbstractCollection&lt;E&gt;"] {
        <<abstract>>
        +add(E e)
        +addAll(Collection c)
    }
    
    class AbstractSetE["AbstractSet&lt;E&gt;"] {
        <<abstract>>
    }
    
    class HashSetE["HashSet&lt;E&gt;"] {
    }
    
    class InstrumentedSetE["InstrumentedSet&lt;E&gt;"] {
        -int addCount
        +getAddCount() int
        +add(E e) boolean
    ...
    AbstractCollectionE <|-- AbstractSetE
    AbstractSetE <|-- HashSetE
    HashSetE <|-- InstrumentedSetE
```
</Slide>
...
classDiagram
    ...
    SubmissionService o-- BuildStrategy
    style BuildStrategy fill:#2d4a5a,stroke:#4ac,stroke-width:2px,stroke-dasharray: 5 5
```

<p style={{fontSize: '0.75em', marginTop: '0.5em', color: '#8BC34A'}}>
