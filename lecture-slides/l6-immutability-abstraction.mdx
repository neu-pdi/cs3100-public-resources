...
<Img
  src="/img/lectures/web/l7-hyrum.jpg"
  alt="Hyrum Wright"
  style={{maxHeight: '30vh', borderRadius: '8px', display: 'block', margin: '0 auto'}}
/>
...
class Counter {
    int count = 0;  // No private keyword exists!
...
<ol style={{fontSize: '0.85em', marginTop: '1em'}}>
  <li><strong>Access modifiers</strong> — control who can see what</li>
  <li><strong>Immutable classes</strong> — prevent state changes after construction</li>
  <li><strong>Sealed classes</strong> — control who can extend a type</li>
  <li><strong>The module system</strong> — hide entire packages from library consumers</li>
</ol>
...
## Reference Fields Can Leak Mutability
...
```java
public final class PhoneNumber {
    private final short[] number;  // Reference!

    public PhoneNumber(short[] number) {
        this.number = number;  // Stores reference!
    }

    public short[] getNumber() {
        return number;  // Returns reference!
    }
}
```
...
- Storing the reference means storing the *same* array
- Caller can still modify it
- `final` only prevents reassigning the reference, not mutating the contents

**The same issue in getNumber():**
- Returning the internal array exposes it
- Caller can modify our internal state
...
<ol style={{fontSize: '0.85em'}}>
  ...
  <li>For reference fields, make <strong>defensive copies</strong> in constructors and getters</li>
</ol>
...
<Img
  src="/img/lectures/web/l6-sealed-vip.webp"
