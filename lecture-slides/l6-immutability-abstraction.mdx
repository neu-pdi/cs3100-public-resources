...
public class PhoneNumber {
  ...
  public PhoneNumber(int[] digits) {
    ...
    if (digits.length != 10) {
      throw new IllegalArgumentException("numbers length must be 7");
    }
...
public class PhoneNumber {
  ...
  public PhoneNumber(int[] digits) {
    ...
    this.digits = Arrays.copy(digits);
  }
...
flowchart LR
    digits["digits<br/>(local variable)"]
    field["phoneNumber.digits<br/>(private field)"]
    array1["{5,1,0,4,3,0,2,5,5,5}"]
    array2["{5,1,0,4,3,0,2,5,5,5}"]
...
```java
  public static void main(String[] args) {
    ...
    PhoneNumber phoneNumber = new PhoneNumber(digits);
    digits[0] = -20; // does not affect phoneNumber.digits
    phoneNumber.getDigits()[0] = -20; // does not affect phoneNumber.digits
  }
```
</div>
