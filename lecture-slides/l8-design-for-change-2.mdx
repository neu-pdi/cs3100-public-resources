...
<Img
  src="/img/lectures/web/l8-dep-inversion.webp"
  prompt="An illustration showing Dependency Inversion Principle using a power cord metaphor.

LEFT SIDE (Bad - 'Direct Dependency'): A laptop with its power cord's bare wires shoved directly into a hole in the wall - no outlet, no plug, just exposed copper wire jammed into drywall. The wall has 'MySQL' written on it. Sparks fly. Thought bubble: 'I can ONLY work with this wall!'

RIGHT SIDE (Good - 'Depend on Abstraction'): The same laptop with a standard power plug (labeled 'Database'). Multiple standard wall outlets below labeled 'MySQL', 'PostgreSQL', 'MongoDB'. The laptop can plug into any of them. Thought bubble: 'I work with any outlet!'

The contrast: direct wiring = can't change, standard plug = swap freely."
  alt="Left: Laptop with bare wires jammed directly into wall labeled MySQL - sparks flying. Right: Laptop with standard plug can connect to any of several outlets (MySQL, PostgreSQL, MongoDB)."
/>
