...
- Subclasses must honor the contract of the parent
...
public class DimmableLight extends Light {
    ...
    public void turnOn() {
        super.turnOn();
        // Contract honored: light turns on
...
<Img
  src="/img/lectures/web/l8-dependency-inversion.webp"
  prompt="An illustration showing Dependency Inversion Principle using a power adapter metaphor.

LEFT SIDE (Bad - 'Direct Dependency'): A laptop (labeled 'PawtograderSystem') with its power cord hardwired directly into a specific wall outlet labeled 'MySQL'. The cord is permanently attached with welds/solder directly into the terminals of the outlet. A thought bubble from the laptop: 'I can ONLY work with MySQL!' If you want to use a different database, you'd have to cut the cord.

RIGHT SIDE (Good - 'Depend on Abstraction'): The same laptop, but now it has a standard power plug (labeled 'Database interface'). Below are multiple adapters/outlets: 'MySQL', 'PostgreSQL', 'MongoDB', all with the same standard socket. The laptop can plug into any of them. Thought bubble: 'I work with any Database!'

The message: high-level modules should depend on interfaces, not concrete implementations."
  alt="Left: Laptop hardwired directly to MySQL outlet - can only work with MySQL. Right: Laptop with standard 'Database interface' plug can connect to MySQL, PostgreSQL, or MongoDB adapters. Message: depend on abstractions, not concretions."
/>
...
## Poll: Which SOLID principle is violated?
...
<Img
  src="/img/lectures/web/l8-composition-inheritance.webp"
  prompt="A side-by-side comparison illustration showing Composition vs Inheritance.

LEFT SIDE (Inheritance - labeled 'IS-A'): A family tree structure where a child class is tightly bound to its parent. The parent class is shown as a large figure with the child physically attached/growing from it like a conjoined twin. Any change to the parent (shown with a hammer/modification) causes cracks to appear in the child too. The connection is rigid and permanent. Label: 'Tight Coupling - Changes Propagate'

RIGHT SIDE (Composition - labeled 'HAS-A'): Separate, independent objects connected by flexible cables or plugs. The main object 'has' references to helper objects, each in their own box. You can unplug one helper and plug in a different one without affecting the others. Label: 'Loose Coupling - Swap Freely'

The contrast shows why composition is generally more flexible than inheritance."
  alt="Left: Inheritance shown as parent-child physically connected; changes to parent crack the child. Right: Composition shown as separate objects connected by flexible plugs - can swap helpers freely without affecting others."
/>
...
<p style={{fontSize: '0.85em'}}>
  Imagine we want a `Set` that counts how many elements have been added:
</p>
...
<Img
  src="/img/lectures/web/l8-fragile-base.webp"
...
<Img
  src="/img/lectures/web/l8-inheritance-vs-composition-detail.webp"
  prompt="A detailed comparison showing the call flow in inheritance vs composition for the InstrumentedSet example.

LEFT SIDE (Inheritance - PROBLEMATIC):
Show a vertical call stack:
1. Client calls addAll(['A','B','C'])
2. InstrumentedHashSet.addAll() runs, count += 3
3. Calls super.addAll()
4. HashSet.addAll() internally calls this.add('A')
5. Which goes to InstrumentedHashSet.add(), count++
6. Repeat for 'B' and 'C'
Final count: 6 (WRONG!)
Show the 'this' pointer bouncing back up to the subclass unexpectedly.

RIGHT SIDE (Composition - CORRECT):
Show a vertical call stack:
1. Client calls addAll(['A','B','C'])
2. InstrumentedSet.addAll() runs, count += 3
3. Calls s.addAll() on the WRAPPED set
4. HashSet.addAll() internally calls this.add('A')
5. But 'this' is the HashSet, not InstrumentedSet!
6. Our add() is never called by the wrapped set
Final count: 3 (CORRECT!)

Use arrows and different colors to show the call flow clearly."
  alt="Left: Inheritance call flow shows addAll incrementing count, then super.addAll calls back to overridden add() 3 more times - count becomes 6. Right: Composition call flow shows addAll incrementing count, then s.addAll stays inside wrapped HashSet - count stays 3."
/>
...
<p style={{fontSize: '0.85em'}}>
  The Java I/O streams API extensively uses the Decorator pattern:
</p>
...
// Add buffering (decorator #1)
InputStream bufferedStream = new BufferedInputStream(fileStream);

// Add ability to read primitives (decorator #2)
DataInputStream dataStream = new DataInputStream(bufferedStream);

// Now we can read int, double, etc. with buffering!
int value = dataStream.readInt();
```

<Img
  src="/img/lectures/web/l8-decorator-io.webp"
  prompt="An illustration showing Java I/O streams as a series of nested wrappers, like Russian nesting dolls or layered gift wrapping.

Show three concentric layers:
- INNERMOST: A simple file icon labeled 'FileInputStream' - 'Reads bytes from file'
- MIDDLE LAYER: A buffer/cache icon wrapping around it, labeled 'BufferedInputStream' - 'Adds buffering for efficiency'
- OUTERMOST: A calculator/data type icon wrapping around that, labeled 'DataInputStream' - 'Adds readInt(), readDouble(), etc.'

Arrows show data flowing from the file, through each layer, to the application. Each layer adds its enhancement without knowing about the others.

Alternative visualization: A factory assembly line where raw bytes enter, get buffered at station 1, get parsed into primitives at station 2, and emerge as usable data."
  alt="Russian nesting doll visualization: FileInputStream (innermost, reads bytes) wrapped by BufferedInputStream (middle, adds buffering) wrapped by DataInputStream (outermost, adds readInt/readDouble). Data flows through each layer."
/>
...
## Decorators Support All Four SOLID Principles

<ul style={{fontSize: '0.85em'}}>
  <li><strong>Add responsibilities dynamically:</strong> Stack decorators at runtime</li>
...
<p style={{fontSize: '0.85em', marginTop: '1.5em', fontWeight: 'bold'}}>
...
## Choose Inheritance Only When It Clearly Fits
...
<Img
  src="/img/lectures/web/l8-takeaways.webp"
  prompt="A summary illustration with four key takeaways arranged in quadrants:

TOP LEFT - 'SOLID Principles': Five interlocking gears labeled S, O, L, I, D working together smoothly. Caption: 'Guidelines for changeable design'

TOP RIGHT - 'Composition over Inheritance': A flexible modular construction vs a rigid family tree. Caption: 'Favor HAS-A over IS-A'

BOTTOM LEFT - 'Safe Inheritance': A blueprint/documentation with checkmarks next to 'Document', 'Design', 'Or make final'. Caption: 'Design for it or prohibit it'

BOTTOM RIGHT - 'Decorator Pattern': Nested wrapper layers adding capabilities. Caption: 'Add behavior without modifying'

CENTER: A badge or seal saying 'Design for Change' with all four concepts contributing to it."
  alt="Four quadrants summarizing lecture: SOLID Principles (five interlocking gears), Composition over Inheritance (modular vs rigid tree), Safe Inheritance (documented blueprint), Decorator Pattern (nested wrappers). Center badge: 'Design for Change'."
/>
