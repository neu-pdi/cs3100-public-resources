...
public class CreateSubmissionHandler implements RequestHandler<Request, Response> {
    ...
    public Response handleRequest(Request request, Context context) {
        ...
        OIDCClaims claims = verifyGitHubOIDC(request.getHeader("Authorization"));   // Verify caller
        Submission sub = db.insertSubmission(body.assignmentId(), claims.repo());   // Do the work
        return Response.ok(new SubmissionResponse(sub.id(), storage.getUrl(...)));  // Return output
    }
}
```
...
**② Stateless:**
- "Notice we create the database and storage clients inside the handler"
