...
- **Defining Serverless + FaaS** (~8 min) — includes Lambda code examples with S3 triggers and KV stores
- **Architecture Comparison** (~8 min) — universal examples: image upload, email, video
- **Requirements Fit** — good fit vs poor fit (~3 min)
...
For users to access your image resizer, you need an <strong>HTTP server</strong> that listens for requests. Now look how much code you need:
</p>

<div style={{fontSize: '0.36em', marginTop: '0.3em'}}>
...
<div className="fragment">
<p style={{fontSize: '0.65em', marginTop: '0.3em', color: '#FF9800'}}>
...
<p style={{fontSize: '0.78em'}}>
Your <code>ImageResizeServer</code> needs to run somewhere. Running code that serves users around the clock requires far more than just "a computer." This is what we call <strong>infrastructure</strong>.
</p>
...
<strong>Pawtograder:</strong> Uses PostgreSQL — we need queries like "find all submissions by this student across all assignments" and "calculate average scores grouped by section." Relational databases shine here. Uses Key-Value store for rate limiting requests to GitHub.
</p>
</div>
...
**Why Pawtograder uses a KV store (hosted Redis):**
...
## Lambda + KV Store: Rate Limiting with External State

<p style={{fontSize: '0.78em'}}>
Serverless functions are <strong>stateless</strong> — they can't remember anything between invocations. For state like rate limits, you need an external store. Remember the <strong>Key-Value stores</strong> from our building blocks? This is exactly what they're for.
</p>
...
public class RateLimitedApiHandler implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    // Redis KV store client — connects over HTTPS, no persistent connection needed
    private final JedisPooled redis = new JedisPooled("https://your-redis-url.example.io", 6379,
        new JedisClientConfig().password("your-redis-token"));
...
<strong>Why a managed KV store?</strong> Traditional Redis needs a persistent connection. Serverless-friendly KV stores (Upstash, AWS ElastiCache, etc.) are HTTP-based — perfect for serverless where connections are expensive and functions are ephemeral.
</p>
</div>
...
- Managed KV stores (Upstash, ElastiCache): HTTP API, pay per request, no connection management
...
**Cost model:**
- Managed KV stores: ~$0.20 per 100K commands
...
- Small visual: API gateway with counter badge, connected to Redis/KV store icon
- Caption: 'External state in KV store'
...
Color scheme: Green (#4CAF50) for good fit, Red (#f44336) for poor fit, clean professional style."
  alt="Split illustration showing serverless fit. LEFT (green): Image resize (S3 trigger), Welcome emails (database trigger), API rate limiting (KV store), Profile uploads at deadline (bursty traffic). RIGHT (red): Video encoding (timeout), Multiplayer games (cold starts), In-memory cache (stateless problem), High-frequency trading (sustained load). Center shows the three key questions: scaling, latency, ops."
/>
...
- "Rate limiting: We used a KV store because functions are stateless — external store solves it."
...
- "In-memory cache: Function memory disappears. That's WHY we used a KV store for rate limiting."
...
<Img
  src="/img/lectures/web/l21-information-hiding-scale5.webp"
  prompt="An educational illustration in a clean, modern flat-art style (NOT pixel art). No title text.
...
|-----------|--------------|------------------|
| **Web API** | Serverless functions | Traffic is bursty (assignment deadlines), small team can't manage servers |
| **Grading execution** | GitHub Actions | Jobs run for minutes, needs isolation for student code |
| **Business logic** | PostgreSQL database | Data consistency matters, database is always-on (no cold starts) |
| **Rate limiting** | Redis (fast key-value store) | Track API usage without slowing down requests |
...
Domain logic behind ports. Different adapters (Edge Functions, background workers) can implement the same interface.
...
**Business logic (always-on database):**
- Deadline enforcement needs to be fast and reliable
- Cold starts would slow down every grade visibility check
- Database is always warm — no startup delay
- → Critical paths avoid serverless cold starts

**Rate limiting (KV store):**
- Edge Functions can't hold state in memory
- Need fast external storage for rate limit counters
- Redis/KV stores are purpose-built for this
- → Managed KV service
...
3. **Serverless = technical partitioning with a vendor.** You write functions; they operate infrastructure.
