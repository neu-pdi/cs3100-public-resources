...
**While Debugging: Hand-Sketch**

Quick ASCII art is fastest:
```
...
When sharing or preserving:
- [mermaid.live](https://mermaid.live) for live editing
- Ask AI to generate from code
- Verify and tweak the output
- See Tools & Resources for syntax
...
<p style={{fontSize: '0.85em', marginTop: '1em', color: '#9370DB'}}>
<strong>Pro tip:</strong> Ask AI to generate Mermaid diagrams from code, then verify them yourself.
</p>

<aside className="notes">
**The workflow:**
1. **Hand-sketch** while debugging (fast, flexible)
2. **Formalize with Mermaid** when documenting (shareable, version-controlled)
3. **Use AI** to convert sketches to Mermaid (saves time on syntax)
...
**Example 1: Understanding CookbookImpl construction flow**

```
Prompt: "Create a sequence diagram showing how CookbookImpl is created 
from JSON using Jackson. Include the @JsonCreator constructor, the 
isBlank() validation for optional fields, and List.copyOf() for recipes."
```

✅ **Good use of AI:** Complex flow with multiple steps, saves time on Mermaid syntax

</div>

<div style={{fontSize: '0.75em', marginTop: '0.5em'}}>

**Example 2: Tracing data flow through UserLibraryImpl**

```
Prompt: "Show the data flow when UserLibraryImpl.findRecipesByTitle() 
searches across collections. Include the stream operations: stream(), 
flatMap(), filter(), and toList()."
```

✅ **Good use of AI:** Multi-step pipeline, AI can visualize the stream transformations

</div>

<aside className="notes">
**These are GOOD uses of AI for diagrams:**
...
## When to Navigate Manually Instead
...
- **Single method lookup:** "What does `containsRecipe()` do?"
  - Just Ctrl+Click in your IDE (F12 in VS Code)
  - AI is slower and might hallucinate

- **Finding where a method is called:**
  - Use IDE's "Find Usages" (Alt+F7 in IntelliJ)
  - Shows ALL call sites instantly

- **Checking a simple branch:**
  - Just read the code: `if (containsRecipe(id)) { throw... }`
  - No diagram needed
...
<aside className="notes">
**The key principle: Use the right tool for the job**

**IDE navigation is ALWAYS faster for:**
- Go to definition (one hop)
- Find all usages (comprehensive list)
- Type hierarchy (shows implementations)
- These are instant and accurate

**AI is better for:**
- Generating formal diagrams for documentation
- Understanding complex flows you haven't seen before
- Creating visual aids for team discussion
- Saving time on Mermaid syntax

**Real example from A3:**
- ❌ "How do I add a recipe to a cookbook?" → Just read `CookbookImpl.addRecipe()` (10 lines)
- ✅ "How does JSON deserialization work end-to-end?" → AI can diagram the full Jackson flow

**The workflow:**
1. Use IDE to navigate and understand
2. Hand-sketch if needed for your own notes
3. Ask AI to formalize into Mermaid when sharing/documenting
...
## Effective AI Prompts for A3 Diagrams

<div style={{fontSize: '0.7em'}}>

**Effective Prompt (specific, with context):**

> "I'm working on A3's JsonRecipeCollectionRepository. Create a sequence diagram showing:
> 1. The `save(collection)` method
> 2. How it uses ObjectMapper to serialize to JSON
> 3. How @JsonTypeInfo adds the 'type' field to distinguish CookbookImpl vs PersonalCollectionImpl
> 4. Writing to file with Files.writeString()
>
> Use Mermaid syntax with participants: Test, Repository, ObjectMapper, FileSystem"

</div>

<div style={{fontSize: '0.7em', marginTop: '0.8em'}}>
...
<div style={{backgroundColor: 'rgba(244, 67, 54, 0.1)', padding: '0.8em', borderRadius: '8px'}}>

**❌ Ineffective: Vague prompt**

> "Diagram the repository pattern"

**Result:**
- Generic UML with Repository interface
- Not specific to YOUR codebase
- Doesn't help understand A3

**Time wasted:** 5 minutes back-and-forth

</div>

<div style={{backgroundColor: 'rgba(76, 175, 80, 0.1)', padding: '0.8em', borderRadius: '8px'}}>

**✅ Effective: Specific prompt**

> "Show data flow through CookbookImpl constructor: `@JsonProperty` parameters → blank string normalization → `List.copyOf()` defensive copy → field assignment"

**Result:**
- Diagram matches YOUR code
- Shows the blank string edge case
- Explains defensive copying

**Time saved:** Got useful diagram first try
...
**❌ Wrong tool: AI for simple lookup**

> "What does the `addRecipe` method do?"

**Result:**
- AI reads the code and explains it
- You could have just read it yourself
- Slower than Ctrl+Click

**Why wrong:** Simple navigation task

</div>

<div style={{backgroundColor: 'rgba(76, 175, 80, 0.1)', padding: '0.8em', borderRadius: '8px'}}>

**✅ Right tool: IDE for navigation**

> Press F12 on `addRecipe`

**Result:**
- Instantly see implementation
- See all parameters and return type
- Can step through with debugger

**Why right:** Direct, instant, accurate
...
## Connection: L13's 6-Step Workflow = Scientific Debugging

<p style={{fontSize: '0.85em'}}>
Remember the 6-step workflow from <strong>L13: AI Coding Assistants</strong>? It maps directly to scientific debugging:
</p>

<div style={{fontSize: '0.75em', marginTop: '0.5em'}}>

| L13 Workflow Step | Debugging Equivalent |
|-------------------|---------------------|
| **1. Identify** context AI needs | **Observe** the bug, gather symptoms and context |
| **2. Engage** with effective prompts | **Hypothesize** about the root cause |
| **3. Evaluate** AI output critically | **Predict** what evidence would support/refute hypothesis |
| **4. Calibrate** toward better results | **Test** your hypothesis with debugger or tests |
| **5. Tweak** generated artifacts | **Analyze** results, refine hypothesis if needed |
| **6. Finalize** and document | **Fix** and verify, document the root cause |

</div>

<p style={{fontSize: '0.85em', marginTop: '1em', color: '#9370DB', fontWeight: 'bold'}}>
Same framework, same discipline: <strong>systematic thinking beats guessing</strong>.
</p>

<aside className="notes">
**The key connection:**
- L13's workflow isn't just for AI—it's a general problem-solving framework
- Debugging IS hypothesis-driven problem solving
- The skills transfer directly

**Why this matters:**
- You're not learning separate techniques—you're mastering ONE systematic approach
- This same framework applies to: debugging, AI collaboration, requirements gathering, design exploration
- Software engineering is about disciplined thinking, not memorizing tricks

→ **Transition:** Let's work through a concrete debugging example...
</aside>
...
| Does the error message disappear? ✓ | Will this fix cause regressions elsewhere? |
...
**The Pattern:**
1. Ask AI "why doesn't this work?"
2. AI suggests a change
3. Try it, see if error changes
4. Repeat until it "works"
5. Never understand what the bug was
...
**The Problem:**
- You've hidden the bug, not fixed it
- You can't explain the fix to anyone
- Similar bugs will stump you again
- The "fix" may introduce new bugs
- You've created maintenance debt
...
<p style={{fontSize: '0.9em', marginTop: '1em', color: '#e74c3c', fontWeight: 'bold'}}>
Just like vibe coding: code you don't understand becomes debt you can't repay.
</p>

<aside className="notes">
**Connection to L13's "vibe coding":**
- Same trap, different context
- Accepting AI output without understanding
- Works short-term, fails long-term

**Why this happens:**
- It's tempting when you're stuck
- AI can make errors disappear without explaining why
- You feel productive ("I'm making progress!")
- But you're not learning, just accumulating debt

**The alternative:**
- Use AI to UNDERSTAND, not just to "fix"
- Ask "why might this be happening?" not "fix this"
- Verify you can explain the fix before accepting it

→ **Transition:** So how DO you use AI effectively for debugging?
</aside>
