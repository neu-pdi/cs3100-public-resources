...
- Code file with many print statements scattered throughout, highlighted in yellow
- A developer adding yet another println, looking frustrated
- Below: A massive scrolling terminal/log output with hundreds of lines
- Developer squinting at the log, trying to find the relevant line
- Magnifying glass searching through noise
- Multiple run cycles shown (edit → run → read logs → repeat)
- Visual feeling: cluttered, noisy, indirect, guessing

RIGHT SIDE - DEBUGGER (clean, direct):
- Same code but clean, no print statements added
- A clear breakpoint marker (red dot) on one line
- Execution paused at that line (highlighted)
- A clean variables panel showing ALL values: tag=true, quote=false, out='foo', c='>'
- Step controls visible (step over, step into, continue)
- Developer looking directly at the state, confident expression
- Single workflow: set breakpoint → run → inspect
- Visual feeling: focused, direct, complete visibility

CENTER - Arrow or VS symbol between them

VISUAL CONTRAST:
- Left: messy, many artifacts, indirect observation
- Right: clean, focused, direct observation
- The debugger side should look obviously better/easier

Style: Clean educational illustration. The contrast should be immediately obvious - println is noisy and indirect, debugger is clean and direct."
/>
