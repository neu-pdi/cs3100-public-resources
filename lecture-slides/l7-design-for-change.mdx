---
sidebar_position: 7
title: "LS7: Changeability II: Coupling and Cohesion"
...
<ol style={{fontSize: '0.75em', textAlign: 'left'}}>
  ...
  <li>Define and recognize cases of coincidental, logical, temporal, procedural, communicational, sequential, and functional cohesion</li>
...
<PollSlide choices={["Abstraction", "Encapsulation", "Inheritance", "Litigation", "Marble", "Polymorphism", "Polyphemus"] } />
...
<p style={{fontSize: '0.65em', marginTop: '0.5em', color: '#888'}}>
  <em>Why "stamp"? Like being coupled to the exact stamp on an envelope with a letter inside—you're coupled to its specific format, which might change.</em>
</p>
...
{/* Eliminate picture so content fits on slide.
<Img
...
<p style={{fontSize: '0.7em', marginTop: '0em'}}>
  This works if we want the caller to determine how to send the notification. But there's an even better approach—the <strong>Strategy pattern</strong>—which we'll see after discussing cohesion.
</p>
...
<p style={{fontSize: '0.9em', color: '#f44336', fontWeight: 'bold'}}>
...
<Img src="/img/lectures/web/l7-reflection-meme.webp"
  alt="Meme with heading 'Me after learning reflection in Java'.
...
<p style={{fontSize: '0.7em', marginTop: '1em', color: '#FF9800'}}>
...
<p style={{fontSize: '0.7em'}}>
  Parts are grouped because they are executed at the <strong>same time</strong>.
</p>
...
<p style={{fontSize: '0.85em'}}>
  Parts operate on the <strong>same data</strong>. (Same example as procedural cohesion!)
</p>
...
<p style={{fontSize: '0.8em', marginTop: '0.5em', color: '#8BC34A'}}>
...
<p style={{fontSize: '0.8em', marginTop: '0.5em'}}>
  <strong>Key:</strong> <code>gradeSubmission</code> <em>must</em> be called after <code>runTests</code> and <code>lintSubmission</code>.
</p>
...
classDiagram
  direction BT
    ...
    SubmissionService --o Pawtograder
    EmailService --o Pawtograder
    CanvasService --o Pawtograder
```
...
<p style={{fontSize: '0.85em'}}>
  Recall the <strong>Strategy pattern</strong>: encapsulate interchangeable algorithms behind a common interface. Each variant gets its own class.
</p>
...
<p style={{fontSize: '0.75em', marginTop: '0.5em', color: '#8BC34A'}}>
  The choice of <em>which</em> notification to send is now made when the <code>NotificationSender</code> is created—not buried inside the method.
</p>
...
**Strategy** is a design pattern that *uses* polymorphism to make algorithms interchangeable
by holding a reference to the concrete subtype.
...
**When people say "use polymorphism instead of switch":**
They usually mean Strategy specifically, but the underlying mechanism is polymorphism.
</aside>
