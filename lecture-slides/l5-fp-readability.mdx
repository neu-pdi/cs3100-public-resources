---
sidebar_position: 5
title: "LS5: Functional Programming and Readability"
...
<PollSlide choices={["Python", "Java", "I like them equally", "I don't know Java well enough yet to have an opinion"]}
/>
...
<PollSlide bottomText="Remember to state the language." />

</Slide>

{/* ============================================ */}
{/* BRIDGE FROM L4: COMPARETO REVIEW */}
{/* ============================================ */}

<Slide>

## Review: compareTo Defines Natural Ordering

<p style={{fontSize: '0.9em'}}>
  Last time, we learned that <code>Comparable</code> lets a class define its <strong>natural order</strong>:
</p>
...
// Now we can sort!
Collections.sort(lights);  // Uses compareTo automatically
```

<p style={{fontSize: '0.85em', marginTop: '0.5em', color: '#27ae60'}}>
  This contract is elegant, well-specified, and has worked for decades.
</p>

<aside className="notes">
**Quick review from L4:**
- `Comparable<T>` interface with one method: `compareTo(T other)`
- Returns negative (less than), zero (equal), positive (greater than)
- Enables `Collections.sort()`, `TreeSet`, `TreeMap`

**The contract is great:**
- Well-defined specification
- Symmetric: `sgn(x.compareTo(y)) == -sgn(y.compareTo(x))`
- Transitive: if a > b and b > c, then a > c
- Used throughout Java since 1.2 (1998!)

**But there's a limitation coming...**

→ **Transition:** But what if we need MORE than one ordering?
</aside>
...
## The Limitation: Only ONE Natural Order

<p style={{fontSize: '0.9em'}}>
  What if users want to sort lights <strong>different ways</strong>?
</p>

<ul style={{fontSize: '0.85em', marginTop: '0.5em'}}>
  <li>By <strong>id</strong> (alphabetically) — for inventory lists</li>
  <li>By <strong>brightness</strong> — to find the dimmest/brightest</li>
  <li>By <strong>location</strong> — to group by room</li>
  <li>By <strong>power consumption</strong> — for energy reports</li>
</ul>

<p style={{fontSize: '0.9em', marginTop: '1em', color: '#e74c3c'}}>
  <code>compareTo</code> can only define <strong>one</strong> of these!
</p>

```java
// Which ordering should compareTo use?
// We can only pick ONE as the "natural" order
public int compareTo(DimmableLight other) {
    return ???  // by id? by brightness? by location?
}
```

<aside className="notes">
**The problem:**
- `compareTo` is baked into the class
- The class author picks ONE ordering
- But different use cases need different orderings!

**Real-world examples:**
- Files: sort by name? by date? by size? by type?
- Students: sort by name? by ID? by GPA? by graduation year?
- Products: sort by price? by rating? by popularity?

**The question:**
- How do we support MULTIPLE orderings?
- Without modifying the class each time?
- This is where `Comparator` comes in...

→ **Transition:** We need a way to specify the ordering SEPARATELY from the class...
</aside>
...
## The Solution: Comparator — Pass the Ordering as an Argument

<p style={{fontSize: '0.9em'}}>
  The <code>Comparator</code> interface wraps comparison logic in an object:
</p>

```java
// Sort by brightness using an anonymous class
Collections.sort(lights, new Comparator<DimmableLight>() {
    @Override
...
<p style={{fontSize: '0.85em', marginTop: '0.5em', color: '#27ae60'}}>
  Now we can sort by <strong>any</strong> field — without modifying <code>DimmableLight</code>!
</p>

<aside className="notes">
**The Comparator solution:**
- `Comparator<T>` interface with one method: `compare(T o1, T o2)`
- Same return semantics as `compareTo`: negative, zero, positive
- But it's SEPARATE from the class being compared

**The flexibility:**
- Sort by brightness? Write a brightness Comparator
- Sort by location? Write a location Comparator
- The class doesn't need to change!

**But look at that code...**
- 6 lines to express "compare by brightness"
- Most of it is ceremony: `new Comparator...`, `@Override`, braces
- The actual logic is ONE line: `Integer.compare(...)`

→ **Transition:** This works, but... is it readable?
</aside>
...
<p style={{fontSize: '0.9em', marginTop: '1em'}}>
  6 lines of code. Only <strong>one line</strong> expresses the actual intent.
</p>

<p style={{fontSize: '0.85em', marginTop: '0.5em', color: '#e74c3c'}}>
  The <strong>ceremony</strong> (boilerplate) overwhelms the <strong>intent</strong> (compare by brightness).
</p>

<p style={{fontSize: '0.9em', marginTop: '1em', color: '#9370DB'}}>
  Can we do better? First, let's talk about why readability matters...
</p>

<aside className="notes">
**The problem:**
- We solved the "multiple orderings" problem
- But the solution is VERBOSE
- Readers have to hunt for the meaningful part

**What's ceremony vs intent:**
- Ceremony: `new Comparator<DimmableLight>() { @Override public int compare(...`
- Intent: `l1.getBrightness()` vs `l2.getBrightness()`
- The signal is buried in noise

**The promise of this lecture:**
- Java 8+ gives us tools to fix this
- But first, let's understand WHY readability matters
- Then we'll see the elegant solutions

→ **Transition:** Let's talk about why readability is so important...
</aside>
...
<Img src="/img/lectures/web/l5-old-code.png" alt="Meme"/>
...
2. Records & Pattern Matching — for data (DTOs, value objects)
...
## Wait... Why Do We Need a Whole Class for One Function?

<p style={{fontSize: '0.9em'}}>
  Look at that anonymous <code>Comparator</code> again:
</p>
...
<p style={{fontSize: '0.9em', marginTop: '0.5em'}}>
  We don't want a <strong>class</strong>. We just want to pass a <strong>function</strong>.
</p>

<p style={{fontSize: '0.85em', marginTop: '0.5em', color: '#e74c3c'}}>
  <em>\"Given two lights, return which one is greater.\"</em> — that's it!
</p>

<aside className="notes">
**The absurdity:**
- We want to express ONE thing: how to compare two lights
- But Java forces us to create a whole class!
- With `@Override`, method signature, braces...
- All to wrap a single function

**What we WANT to say:**
- "Given l1 and l2, compare their brightness"
- That's a function: two inputs → one output
- Why can't we just write that?

**The core question:**
- What if we could treat functions like values?
- Store them in variables, pass them to methods?
- This is the key insight of functional programming

→ **Transition:** What if functions were objects?
</aside>
...
## What If Functions Were Just... Values?

<p style={{fontSize: '0.9em'}}>
  You can store data in variables and pass it around:
</p>

```java
int brightness = 100;                    // Store an int
lights.get(0).setBrightness(brightness); // Pass it to a method
```

<p style={{fontSize: '0.9em', marginTop: '1em'}}>
  What if you could do the same with <strong>behavior</strong>?
</p>

```java
??? comparison = <<compare two lights by brightness>>;
lights.sort(comparison);  // Pass behavior to a method
```

<p style={{fontSize: '0.85em', marginTop: '0.5em', color: '#9370DB'}}>
  This is the big idea: <strong>functions as first-class values</strong>.
</p>
...
- Same method (sort) can do different things based on what function you pass
...
- Java 8 finally gave us an elegant answer

→ **Transition:** Let's see what this looks like...
</aside>
...
{/* I consider the Strategy slides skippable. --ES */}
<Slide>
...
direction BT
    ...
    style Strategy fill:#2d4a5a,stroke:#4ac,stroke-width:2px,stroke-dasharray: 5 5
```
...
<table>
  <thead>
    <tr><th>Parameters</th><th>Syntax</th><th>Example</th></tr>
  </thead>
  <tbody>
    <tr><td>Zero</td><td><code>() -&gt; body</code></td><td><code>() -&gt; System.out.println("Hi")</code></td></tr>
    <tr><td>One</td><td><code>x -&gt; body</code></td><td><code>s -&gt; s.toUpperCase()</code></td></tr>
    <tr>
      <td>Multiple</td>
...
- While the table used a block body with multiple parameters,
  it can be used with any number of parameters.
...
→ **Transition:** How did the compiler know which method in the Comparator interface
...
<aside className="notes">
**Mireds**
- Short for "micro reciprocal degree"
- A unit of measure for color temperature
- Pronounced MY-red (pun?)
...
<Img src="/img/lectures/web/l5-style.png"
 alt="Hackles cartoon in which anthropomorphic animals argue about style"/>
...
- Technical vs user term (CT vs colorTemperature)
...
<Img src="/img/lectures/web/l5-loop-variable-name.png"
  alt="Tweet by Carla Notarboot: 'My team had a debate over what the best looping variable name is...i won"/>
...
<Img src="/img/lectures/web/l5-long-java-names.jpg"
  alt ="Meme showing extremely wide monitor for Java variable names" />
...
<Img src="/img/lectures/web/l5-naming-is-hard.webp" alt="cartoon"/>
