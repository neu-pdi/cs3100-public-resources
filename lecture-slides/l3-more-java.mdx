...
<Img
  ...
  alt="Concept: 'The Unlabeled Warehouse' (Type-Unsafe Storage)
...
<Img
  ...
  alt="Concept: 'The Generics Rebellion' (Why Java Added Generics)
...
## The Right Data Structure Makes the Job Easy

<Img
  src="/img/lectures/web/l3-toolbox.webp"
  alt="Concept: 'Three Ways to Organize' (Data Structure Choice)
Tagline: 'List for sequences. Set for membership. Map for lookup.'

A clean three-panel comparison in flat infographic style, each panel showing the same four IoT devices (Light, Fan, Thermostat, Sensor) organized differently:

LIST panel: Devices in numbered slots [0], [1], [2], [3]. Caption: 'Ordered by position. Duplicates OK.' Code: 'devices.get(2)' with arrow pointing directly to slot 2. Badge: 'O(1) by index'.

SET panel: Devices scattered without numbers, each unique. A duplicate Light bounces away with an X. Caption: 'Unique only. Order not guaranteed.' Code: 'devices.contains(fan)' with checkmark. Badge: 'O(1) membership'.

MAP panel: Devices in labeled compartments: 'living-room' → Light, 'bedroom' → Fan. Caption: 'Find by key, not position.' Code: 'devices.get(bedroom)' returns Fan. Badge: 'O(1) by key'.

Bottom summary: 'What's your question? By position → List. Already have it? → Set. By name → Map.'"
/>

<aside className="notes">
- **Learning Objective 2:** Recognize and apply Java's core data structures
- The graphic shows a toolbox metaphor—different tools for different jobs
- List = numbered hooks (order matters, access by position)
- Set = magnetic board (each item appears once, duplicates bounce off)
- Map = labeled drawers (find by name/key, not position)
- Key insight: Choosing the right data structure makes code simpler AND faster
- Choose wrong, and you fight your tools instead of solving your problem

**Transition:** Let's look at the actual inheritance hierarchy of these collections.
</aside>
...
<Img
  ...
  alt="Concept: 'The Speed of Finding Things' (Library Search Comparison)
...
for (Light l : lights) {
    if (l.getId().equals("target")) { /* found! */ }
}
...
## Each Linked List Node Points to Its Neighbors

<Img
  src="/img/lectures/web/l3-linkedlist.webp"
  alt="Concept: 'The Chain of Nodes' (LinkedList Structure)
Tagline: 'Direct access to the ends, but follow the chain for everything else.'

A clear technical diagram in blueprint style showing a LinkedList with 4 IoT device elements: 'Light-A', 'Fan-B', 'Light-C', 'Thermo-D'.

The main structure shows a 'LinkedList<IoTDevice>' object on the left with two labeled arrows prominently displayed: 'first' pointing directly to Node A, 'last' pointing directly to Node D, and 'size: 4'. These two pointers are highlighted with a green glow and labeled 'ALWAYS DIRECT ACCESS - O(1)!' to emphasize that the LinkedList maintains constant-time access to both ends. Four Node boxes arranged horizontally, each containing three fields: 'prev' pointer (arrow pointing left, or null for first), 'item' field showing the device ('Light-A', 'Fan-B', 'Light-C', 'Thermo-D'), and 'next' pointer (arrow pointing right, or null for last). Bidirectional arrows clearly show the chain: A↔B↔C↔D. Node A's 'prev' shows 'null', Node D's 'next' shows 'null'.

Below, THREE operations are illustrated with step counts:
1. 'addFirst(newLight)' operation (LEFT, highlighted in bright green): Shows a new green Node X being inserted at the front. A direct green arrow from 'first' pointer to the new node. Only 3 pointer updates needed (first→X, X.next→A, A.prev→X). Large caption: 'ALWAYS O(1)—direct pointer to first!' with '0 hops, just rewire'. Code: 'linkedList.addFirst(newLight) // instant!'
2. 'addLast(newThermo)' operation (RIGHT, highlighted in bright green): Shows a new green Node Y being inserted at the end. A direct green arrow from 'last' pointer to the new node. Only 3 pointer updates needed (last→Y, D.next→Y, Y.prev→D). Large caption: 'ALWAYS O(1)—direct pointer to last!' with '0 hops, just rewire'. Code: 'linkedList.addLast(newThermo) // instant!'
3. 'get(2)' operation (CENTER, in orange): A dotted orange line shows traversing from 'first' through Node A (step 1), then Node B (step 2), finally reaching Node C (step 3). Counter shows '3 hops to reach index 2'. Caption: 'Must walk the chain—O(n)'. Code: 'linkedList.get(2) // 3 pointer hops!'

A 'Key Insight' callout box in green states: 'LinkedList ALWAYS has O(1) addFirst() and addLast() because it stores direct pointers to both ends. No traversal needed—ever!'

A 'Common Misconception' sidebar shows two developers arguing. Developer A: 'LinkedList is faster for insertions!' Developer B points to the diagram: 'Only addFirst/addLast are guaranteed O(1). For add(index, element) in the middle? Finding position 500,000 means 250,000 hops first—then you can insert in O(1).' A performance comparison shows: 'Middle insert: ArrayList O(n) shift but cache-friendly vs LinkedList O(n) traversal + O(1) insert with cache misses. ArrayList usually wins! But addFirst/addLast? LinkedList wins every time.'

Color coding: existing nodes in blue, addFirst/addLast operations in bright green with 'O(1) GUARANTEED' badges, traversal path in orange, misconception callout in red."
/>

<aside className="notes">
Here's the visual structure. Each node contains data plus two pointers. The LinkedList object only knows where the chain starts and ends. To find the middle, you must walk the chain node by node. This is fundamentally different from an array where you can jump directly to any index. The doubly-linked structure means you can walk either direction, and insertions/deletions just require rewiring a few pointers.
</aside>
...
## LinkedList Excels Only as a Queue or Deque

<p style={{fontSize: '0.95em'}}>
  LinkedList shines in <strong>very specific</strong> scenarios:
</p>

<ul style={{fontSize: '0.85em'}}>
  <li><strong>Queue operations</strong> — add at end, remove from front (use as <code>Queue</code>)</li>
  <li><strong>Deque operations</strong> — add/remove at both ends</li>
  <li><strong>Iterator-based modification</strong> — insert/remove while iterating</li>
</ul>
...
// Still use ArrayList for most other cases
List<Light> lights = new ArrayList<>();  // Default choice
```

<p style={{marginTop: '0.5em', fontSize: '0.9em', color: '#9370DB'}}>
  <strong>Rule of thumb: ArrayList unless you specifically need queue/deque behavior</strong>
</p>

<aside className="notes">
So when should you actually use LinkedList? When you're using it as a Queue or Deque—adding at one end and removing from the other. LinkedList implements both Queue and Deque interfaces with true O(1) operations at both ends. It's also useful when you're iterating with an Iterator and need to insert/remove at the current position without index-based access. For everything else—random access, searching, typical list operations—ArrayList wins. This isn't just theoretical; real-world benchmarks consistently show ArrayList outperforming LinkedList for most use cases.
</aside>
...
## Sets Guarantee Uniqueness with O(1) Lookup
...
if (deviceRegistry.containsKey("kitchen-light")) {
    deviceRegistry.get("kitchen-light").turnOn();
}
```
...
## Example: Playlists Need Order and Allow Duplicates
...
## Example: Preventing Double-Votes Requires Uniqueness
...
public boolean recordVote(String oderId, String choice) {
    if (voterIds.contains(oderId)) {
...
## Example: Looking Up Grades by Student ID

<p style={{fontSize: '1em'}}>
  <strong>Scenario:</strong> Look up a student's grade by their student ID
</p>
...
## Example: First-Come-First-Served Processing

<p style={{fontSize: '1em'}}>
  <strong>Scenario:</strong> Process customer support tickets in the order they arrived
</p>
...
supportQueue.offer(new Ticket("Password reset"));
supportQueue.offer(new Ticket("Billing question"));

Ticket next = supportQueue.poll();  // Gets "Password reset"
```
</div>
...
## Example: Grouping Devices by Room
...
<div className="fragment">
<p style={{marginTop: '1em', fontSize: '1em', color: '#9370DB'}}>
  <strong>Use: <code>Map&lt;String, List&lt;IoTDevice&gt;&gt;</code></strong>
</p>
...
<Img
  src="/img/lectures/web/l3-boxing.webp"
  alt="Concept: 'The Gift Wrapping Station' (Primitive Boxing)
Tagline: 'Boxing has costs. The == trap has victims.'

A department store gift-wrapping station in retro 1960s advertising style—cheerful on the surface but revealing problems on closer inspection. Three main areas tell the story:

Primitives ARRIVING ON CONVEYOR: Simple, efficient items arrive on a belt: a bare 'int' (just the number 42, compact and lightweight, with a size label '4 bytes'), a plain 'boolean' (a simple toggle switch labeled 'true'), a naked 'double' (3.14159). They approach a velvet-rope entrance to the 'Generic Collections VIP Lounge' but a bouncer in a tuxedo blocks them, pointing to a sign: 'Objects Only. No Primitives Allowed. List<int> is ILLEGAL.'

WRAPPING STATION (Boxing): Harried attendants frantically wrap each primitive in elaborate decorative boxes. The int 42 becomes an ornate 'Integer' gift box—but the box is visibly 4× larger than the tiny number inside (size comparison: '4 bytes → 16 bytes'). Assembly line shows: int→Integer, double→Double, boolean→Boolean. A meter displays 'Memory Usage: ×4' and 'CPU Overhead: autoboxing in progress'. Discarded wrapping material overflows bins. An attendant's thought bubble: 'A million of these per second in that loop...'

CENTER PANEL - THE TRAP: Two beautifully wrapped Integer boxes sit side by side, both visibly containing '128'. They look absolutely identical. A developer confidently uses '==' to compare them, expecting 'true'. Result display shows: 'false!' with shock lines radiating outward. The developer's face shows horror. Explanation callout: '== compares BOXES (object identity), not contents!' A small 'Cache Zone' area shows Integer values -128 to 127 sharing the same boxes, with annotation: 'Small values are cached—so == sometimes works by accident, making the bug even harder to find!' Code snippets show the trap: 'Integer a = 128; Integer b = 128; a == b // false!' vs 'Integer c = 1; Integer d = 1; c == d // true (cached!)'. Giant warning sign: 'ALWAYS use .equals() for wrapper comparison!'

Wrapped objects RETURNING FROM COLLECTIONS: Wrapped objects return from Collections for actual computation. An unwrapping station tears off packaging to extract primitives. More waste accumulates. Code shown: 'int sum = 0; for (Integer i : list) { sum += i; } // unbox million times!'

BOTTOM CORNER: A wise engineer bypasses the entire station, using 'int[]' directly with a smile. Sign points to them: 'When you can, skip the wrapping entirely. Prefer primitives for computation.'"
/>
...
// But wait, this works...
Integer c = 1, d = 1;
System.out.println(c == d);  // true (but by accident!)
```
...
<Img
  ...
  alt="Concept: 'The Universal Plumbing System' (Stream Architecture)
...
<p style={{marginTop: '0.75em', fontSize: '0.85em', color: '#666'}}>
  A Unix heritage from 1969 that every Java program inherits
</p>
...
## Overloading Resolves at Compile Time; Overriding at Runtime

<table style={{margin: '0 auto', fontSize: '0.85em'}}>
  <thead>
    <tr>
      <th style={{padding: '0.5em'}}>Overloading</th>
      <th style={{padding: '0.5em'}}>Overriding</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td style={{padding: '0.5em'}}>Same name, <strong>different parameters</strong></td>
      <td style={{padding: '0.5em'}}>Same name, <strong>same parameters</strong></td>
    </tr>
    <tr>
      <td style={{padding: '0.5em'}}>Resolved at <strong>compile time</strong></td>
      <td style={{padding: '0.5em'}}>Resolved at <strong>runtime</strong></td>
    </tr>
    <tr>
      <td style={{padding: '0.5em'}}>Within same class or inherited</td>
      <td style={{padding: '0.5em'}}>Subclass redefines parent method</td>
    </tr>
    <tr>
      <td style={{padding: '0.5em'}}><code>read()</code>, <code>read(byte[])</code></td>
      <td style={{padding: '0.5em'}}><code>Light.turnOn()</code> → <code>DimmableLight.turnOn()</code></td>
    </tr>
  </tbody>
</table>

```java
// Overloading example from InputStream
public abstract int read();              // Read one byte
public int read(byte[] b);               // Read into array
public int read(byte[] b, int off, int len);  // Read portion
```

<aside className="notes">
A quick clarification on terminology that trips people up. Overloading means same method name with different parameter lists—the compiler chooses which to call based on arguments. Overriding means a subclass provides a new implementation of an inherited method—the JVM chooses at runtime via dynamic dispatch. InputStream shows overloading: three read methods with different signatures. Our DimmableLight.turnOn() overriding Light.turnOn() is overriding. Both are useful; don't confuse them.
</aside>
