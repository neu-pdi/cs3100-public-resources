...
<strong>Note:</strong> We won't drill you on memorizing these styles. Real systems are messier than any textbook diagram. The point is learning to <em>reason about quality attributes</em> — styles are just a useful vehicle for comparison.
</p>
</div>

<aside className="notes">
**SET EXPECTATIONS CLEARLY:**
- "You won't be tested on 'draw a hexagonal architecture diagram from memory'"
- Real systems rarely fit neatly into one style — they're hybrids, compromises, and evolved messes
- The ACTUAL skill we're building: reasoning about quality attributes
- "Given this change in requirements, which quality attributes are affected? Which architectural decisions help or hurt?"

**Why teach styles at all if they're idealized?**
- They give us VOCABULARY for discussing tradeoffs
- Comparing "pure" versions makes the tradeoffs visible
- Like physics: we study frictionless planes not because they exist, but because they reveal principles
- Once you understand the principles, you can reason about messy real systems
...
<div className="fragment">
<div style={{padding: '0.5em', border: '2px solid #FF9800', borderRadius: '8px', marginTop: '0.5em', fontSize: '0.68em'}}>

**Why did this structure emerge?** L18 heuristics in action:
- **Rate of Change:** Domain (grading rules) stable; infrastructure (APIs, databases) volatile
- **Testability:** Domain testable without real price feeds or databases
- **ISP:** Each port is a narrow interface — domain doesn't see full infrastructure complexity

</div>
</div>

<aside className="notes">
**This is a REVIEW — but now with the emergence lens!**
- Students learned domain/port/adapter in L16 with the smart home example
- They saw DI wiring in L17 (creation patterns)
- They found service boundaries in L18 (Pawtograder) using four heuristics
- Today: see how those heuristics LED to hex arch — it wasn't a random choice

**The emergence connection:**
- In L16, we motivated hex arch with testability — "how do we test without real infrastructure?"
- In L18, we learned the broader heuristics — rate of change, actor, ISP, testability
- Now: see that hexagonal is what emerges when you apply those heuristics to a domain-centric problem

**Quick check for understanding:**
- "Can someone remind me: what's a port? What's an adapter?"
- "Why does this structure help with testability?"
...
<Img
    src="/img/lectures/web/l19-hexagonal-pawtograder2.webp"
    prompt="A side-by-side comparison showing hexagonal architecture in two systems: Pawtograder's Grading Action and CookYourBooks recipe app. No title.

LEFT SIDE — PAWTOGRADER GRADING ACTION:
A hexagon with the domain core in the center labeled 'OverlayGrader - Pure Grading Logic'. Around the hexagon, ports are shown as socket-like interfaces:
- Left port: 'Builder' (interface icon)
- Right port: 'FeedbackAPI' (interface icon)
- Bottom port: 'SubmissionAPI' (interface icon)

Adapters plugged into each port:
- GradleBuilder plugged into Builder port
- PythonScriptBuilder also available for Builder port
- SupabaseAPI plugged into FeedbackAPI and SubmissionAPI ports

Code snippet visible in the center:
```
grade(solution, submission, config)
// Knows nothing about
// GitHub, HTTP, or databases
```

RIGHT SIDE — COOKYOURBOOKS (PREVIEW):
A smaller hexagon showing how the same pattern applies to a recipe app:
- Center: 'RecipeService - Pure Recipe Logic'
- Ports: 'RecipeRepository', 'UserPreferences', 'ConversionService'
- Adapters: 'PostgresRecipeRepo', 'InMemoryPrefs', 'KingArthurDensityConverter'

BOTTOM CALLOUT: 'Same pattern, different domains. The hexagon emerges when domain logic needs to be testable without infrastructure.'

Style: Clean educational diagram, hexagons clearly showing domain-at-center structure. Use warm colors: purple (#9370DB) for domain, blue (#4A90A4) for ports, green (#4CAF50) for adapters."
    alt="Two hexagonal architectures side by side: Pawtograder (OverlayGrader with Builder, FeedbackAPI ports) and CookYourBooks preview (RecipeService with Repository, Preferences ports). Callout: Same pattern emerges when domain needs infrastructure isolation."
  />

<aside className="notes">
**Walk through Pawtograder's hexagon:**
- **OverlayGrader (center)**: Takes a solution and submission, grades it. Pure domain logic — no HTTP, no GitHub, no Supabase.
...
**The emergence connection:**
- This structure wasn't picked from a menu — it emerged from applying L18 heuristics
- Rate of Change: Domain (grading rules) changes rarely; adapters (Gradle, Supabase) change when technologies change
- Testability: Domain testable by injecting stub adapters — `java -jar grader.jar` runs locally
- ISP: Each port is narrow — Builder doesn't know about API, API doesn't know about building

**Preview of CookYourBooks:**
- Students will apply this pattern themselves in upcoming assignments
- Same structure: RecipeService at center, Repository and Conversion ports, adapters for persistence
- The hexagon emerges in ANY domain-centric system where you need infrastructure isolation

→ **Transition:** Now let's see the adapters in more detail...
</aside>
...
→ **Transition:** All these styles so far describe organization WITHIN a single deployment. What about deployment itself?
</aside>
