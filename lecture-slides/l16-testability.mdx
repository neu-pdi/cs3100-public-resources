...
<div style={{fontSize: '0.75em', marginTop: '0.5em'}}>
  ...
  <p className="fragment" style={{color: '#f44336', margin: '0.25em 0'}}>❌ <code>System.out.println</code> — output disappears into the void</p>
...
<p style={{fontSize: '0.85em', marginTop: '0.5em'}}>
  In L12, we talked about keeping implementation close to the domain model.<br/>
...
A large hexagonal shape labeled 'APPLICATION CORE' at top, 'Domain Logic' below. Inside the hexagon, show clean code snippets representing pure business logic:
- 'if (price > threshold) reducePower()'
- 'ComfortCalculator'
- 'EnergyOptimizer'
- No imports of HTTP, SQL, or hardware libraries visible
Caption inside: 'Pure business rules. No technology.'

EDGES - SIX PORTS (shown as electrical outlet sockets on each hexagon edge):
- Top: 'EnergyPricePort' (interface symbol)
- Top-right: 'SensorPort' (interface symbol)
- Bottom-right: 'DeviceControlPort' (interface symbol)
- Bottom: 'PreferencesPort' (interface symbol)
- Bottom-left: 'WeatherPort' (interface symbol)
- Top-left: 'NotificationPort' (interface symbol)
Each port has a label: 'interface' in small text

OUTSIDE - ADAPTERS (shown as plug cables connecting to external systems):
Production adapters (solid lines):
- 'GridPriceAPIAdapter' → connects to cloud icon labeled 'Energy Grid API'
- 'ZigbeeSensorAdapter' → connects to IoT sensor icon
- 'PostgresPreferencesAdapter' → connects to database cylinder
- 'TwilioNotificationAdapter' → connects to phone icon

Test adapters (dashed lines, friendly green color):
- 'StubPriceAdapter' plugged into EnergyPricePort, labeled 'returns $0.35'
- 'FakeSensorAdapter' plugged into SensorPort, labeled 'returns 72°F'
- 'SpyNotificationAdapter' plugged into NotificationPort, labeled 'records calls'

CALLOUT BOX (bottom right): 'Tests plug in here! Same ports, different adapters.'

STYLE: Clean technical diagram, blueprint aesthetic. Hexagon glows warm amber. Production adapters in blue. Test adapters in green. External systems in gray."
/>
...
void reducesNonEssentialDevices_whenEnergyPriceExceedsThreshold() {
    ...
    devices.addDevice(new Device("living-room-light", essential: false));
    devices.addDevice(new Device("refrigerator", essential: true));
