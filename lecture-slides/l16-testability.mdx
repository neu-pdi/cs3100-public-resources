...
- Hexagonal Architecture / Ports and Adapters (~15 min)
...
**Key theme:** This lecture answers that question. Testability isn't an accident—it's a consequence of good design.
...
- Objective 2: Hexagonal Architecture (~15 min)
...
<p style={{fontSize: '0.9em'}}>
  If tests take 30 minutes, developers won't run them often. If they take 30 seconds, they'll run them after every change.
</p>
...
```java
@Test
void calculatesComfortAction() {
    ComfortCalculator calc =
        new ComfortCalculator();

    assertEquals(HVACAction.HEAT,
        calc.calculateAction(68.0, 72.0));
    // ... milliseconds
}
```
...
-> **Transition:** But fast isn't enough...
</aside>
...
<p style={{fontSize: '0.9em'}}>
  A <strong>flaky test</strong> sometimes passes and sometimes fails without code changes. Flaky tests destroy trust.
</p>
...
|-------|---------|----------|
| System time | `LocalDateTime.now()` changes | Inject `Clock` |
| Random values | Unpredictable behavior | Inject random generator with fixed seed |
| Execution order | Test B needs Test A to run first | Each test sets up own state |
| External services | Network failures, slow responses | Use test doubles |

</div>

<p style={{fontSize: '0.8em', marginTop: '1em', color: '#f44336'}}>
  ⚠ When tests fail randomly, developers start ignoring failures.
</p>

<aside className="notes">
**The trust problem:**
- "Oh that test is flaky, ignore it"
- Now a real bug can hide behind "flakiness"
- Test suite becomes useless

**The fix:**
- All non-determinism comes from injected dependencies
- Tests control those dependencies completely
- Same inputs → same outputs, always

-> **Transition:** Tests must also be independent...
</aside>
...
<p style={{fontSize: '0.9em'}}>
  Each test should run in isolation. If Test B only passes when Test A runs first, you have a problem.
</p>
...
public class DeviceRegistryTest {
    ...
    void registersNewDevice() {
        ...
        // Fails if another test ran first!
    }
}
```
...
- Failures are easier to debug (isolated)

**The pattern:**
- `@BeforeEach` sets up fresh state
- Each test is a self-contained story
- No assumptions about what ran before

-> **Transition:** Finally, tests should be readable...
</aside>
...
<p style={{fontSize: '0.9em'}}>
  Tests serve as documentation. A well-written test tells a story.
</p>

```java
@Test
void sendsAlertWhenTemperatureExceedsSafetyThreshold() {
    // Given: A safety monitor with a 90°F threshold
    ...
    verify(spyAlerts).sendCriticalAlert(
        argThat(alert -> alert.contains("server-room") &&
                         alert.contains("95.0")));
}
```

<p style={{fontSize: '0.8em', marginTop: '0.5em', color: '#4CAF50'}}>
  The method name describes expected behavior. Given/When/Then structure makes the test's purpose clear.
</p>
...
**Also called:**
- Arrange/Act/Assert (same idea)

**Why it matters:**
- Tests fail → you read the test → you understand the bug
- New developer → reads tests → understands the system
