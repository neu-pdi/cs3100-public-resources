...
<p style={{marginTop: '1em', fontWeight: 'bold', color: '#9370DB'}}>
...
<Img
  src="/img/lectures/web/l12-sicp-wizard3.webp"
  prompt="A medieval scholar in robes teaching a young student. The scholar gestures while explaining something important. A speech scroll above reads: 'Programs are meant to be read by humans and only incidentally for computers to execute.'

In the corner, a small brass clockwork automaton labeled 'Cy D. Fect' waits to perform tasks. The student wears a badge reading 'Ben Bitdiddle'.

A glowing Greek letter lambda (λ) floats in the background.

Style: Detailed pen-and-ink illustration with crosshatching, like a woodcut from an old manuscript. Warm sepia and gold tones. Medieval scholarly aesthetic - think illuminated manuscripts meets alchemy."
  alt="SICP-style illustration: A wizard conjures readable code ('Student', 'Submission', 'GradingSession') while apprentices watch and understand. A small automaton waits passively. A lambda symbol glows in the background. Banner reads: 'Written for people to read.'"
/>

<p style={{fontSize: '0.85em', fontStyle: 'italic', textAlign: 'center', marginTop: '0.5em'}}>
"Programs must be written for people to read, and only incidentally for machines to execute."
<br/><span style={{fontSize: '0.8em'}}>— Harold Abelson & Gerald Sussman, <em>Structure and Interpretation of Computer Programs</em>, 1984</span>
</p>
...
**Note:** This is simplified—full model would include RubricItem, InlineComment,
connections between the top and bottom graphs, etc.
...
<aside className="notes">
Not shown is data attached to the states, such as the current level of review.
</aside>
</Slide>
...
<div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5em', fontSize: '0.75em', marginTop: '0.5em'}}>

<div style={{backgroundColor: 'rgba(147, 112, 219, 0.1)', padding: '0.75em', borderRadius: '8px'}}>

**Controller**

- Handles external requests (HTTP, UI events)
- Knows about request/response formats (JSON, HTML)
- Authenticates and authorizes
- Exists at the system boundary

</div>

<div style={{backgroundColor: 'rgba(100, 200, 150, 0.1)', padding: '0.75em', borderRadius: '8px'}}>

**Service**

- Coordinates internal operations
- Knows nothing about HTTP or UI
- Assumes caller is already authorized
- Exists inside the system
- One per business capability
...
<p style={{fontSize: '0.85em', marginTop: '1em', color: '#9370DB'}}>
A controller might call a service, but a service should never call a controller.
</p>
...
**Example flow:**
1. HoldController receives POST /holds
2. Controller authenticates patron, extracts bookId
3. Controller calls HoldService.placeHold(patron, book)
4. Service coordinates: checks patron.canPlaceHold(), creates Hold, sends notification
5. Controller formats Hold as JSON and returns response

**Why separate them?** Services can be reused (CLI, batch jobs, tests) without going through HTTP. Controllers are tied to one interface.
</aside>
...
```mermaid
flowchart TB
    GUI[GUI] -->|"student clicks 'Escalate'"| Controller[RegradeController]
    Controller -->|authenticates student| Controller
    Controller -->|"escalateRequest(request)"| Service[RegradeService]
    Service -->|"canEscalate?"| Domain[RegradeRequest]
    Service -->|"escalate()"| Domain
    Service -->|"notify()"| Notification[NotificationService]
    Controller -->|"update UI"| GUI
```
...
<Img src="/img/lectures/web/l12-single-responsibility-principle.webp"
    alt="Colorful drawing labeled 'Single Responsible Principle.
    On the left are a wooden spoon, whisk, spatula, and knife.
    On the right is a spork with a frowny face.
    Author: Lara Schenck @larasx"
/>
{/* Image source: https://notlaura.com/writing-css-algorithms/ */}
...
<Slide className="small-code">

## Each Class Has One Clear Job

<style>{`
  .small-code .codeBlockContainer_Ckt0 pre,
  .small-code .codeBlockContainer_Ckt0 code {
...
A lightweight text format for representing structured data:
</div>

```json
{
  "name": "Alice Chen",
  "email": "chen.a@university.edu",
  "submissions": [
    {
      "assignment": "Homework 3",
      "submittedAt": "2025-01-28T23:45:00",
      "status": "GRADED"
    },
    { ... }
  ]
...
```
<div style={{fontSize: '0.75em'}}>
**Serialization:** converting an object to JSON text (to save or send)

**Deserialization:** converting JSON text back to an object (to load or receive)

**Common uses:** saving to files, databases, sending over networks (APIs)
</div>
...
## 16-bit Overflow
    ...
    "
    />
<p style={{fontSize: '0.6em', color: '#666', textAlign: 'center'}}>
  Source: <a href="https://xkcd.com/571/">xkcd.com/571</a> by Randall Munroe (CC BY-NC 2.5)
</p>
</Slide>
...
<Img src="/img/lectures/web/l12-weeks-of-coding-save-hours-of-designing.jpg" alt="Humorous image about the importance of design: 'Weeks of coding can save you hours of designing'" />
