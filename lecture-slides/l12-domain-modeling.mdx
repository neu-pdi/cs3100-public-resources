...
```
Service becomes a "logic hog" that pulls data from anemic objects.
...
ResponseEntity escalate(@PathVariable String id, ...) {
  RegradeRequest req = repo.findById(id);

  // Business logic IN the controller!
  if (req.getResponses().isEmpty()) {
    throw new BadRequestException("No responses");
  }
  if (req.getStatus() == RESOLVED) {
    throw new BadRequestException("Already resolved");
  }
  RegradeResponse last = req.getResponses()
      .get(req.getResponses().size() - 1);
  if (hoursBetween(last.getCreatedAt(), now()) < 24) {
    throw new BadRequestException("Wait 24h");
  }

  req.setStatus(ESCALATED);  // More logic...
  repo.save(req);
  return ok(req);
}
```
...
@PostMapping("/regrade-requests/{id}/escalate")
ResponseEntity escalate(@PathVariable String id, ...) {
  Student student = authService.authenticate(token);
  RegradeRequest req = regradeService.findById(id);

  if (!req.getStudent().equals(student)) {
    throw new UnauthorizedException();
  }

  if (!req.canEscalate()) {  // Domain decides!
    throw new EscalationNotAllowedException();
  }

  req = regradeService.escalateRequest(req);
  return ok(toJson(req));  // Convert to JSON for response
}
```
...
<aside className="notes">
**The contrast:**
- Left: Controller is 30+ lines, full of business logic, hard to test
- Right: Controller is thin—just wiring. Business logic is in `canEscalate()`

**Connection to L8 (Single Responsibility):**
- Left: Controller has TWO jobs—HTTP handling AND business rules
- Right: Controller has ONE job—coordinate the request/response flow
