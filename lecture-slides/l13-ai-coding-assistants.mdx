...
**Structure:**
- What is an LLM? Context is Everything (~5 min)
- AI Coding Assistants and Tab Completion (~5 min)
- The Tool Ecosystem: Providers vs Builders (~7 min)
- The 6-Step Human-AI Collaboration Workflow (~5 min)
- When to Use AI (and When Not To) (~8 min)
- Live Demo: Domain Modeling with GitHub Copilot (~20 min)
- Key Takeaways (~5 min)
...
<Img
  src="/img/lectures/web/l13-context-window.webp"
  prompt="Concept: 'Context Is Everything - The Same Question, Different Answers'. A dramatic before/after comparison showing how context transforms LLM output quality. TOP HALF - 'WITHOUT CONTEXT' (labeled 'Vague Input → Generic Output'): A small prompt box contains: 'Write a good lecture on AI coding assistants for CS3100'. Arrow points to OUTPUT: A generic, bland lecture outline that could apply to any course. Labels indicate: 'Generic structure', 'No course-specific connections', 'Could be from anywhere on the internet'. The output is shown as mediocre/gray. BOTTOM HALF - 'WITH CONTEXT' (labeled 'Rich Input → Tailored Output'): A LARGE prompt box contains: '📄 40,000 words of course notes on specs, requirements, design for change... 📄 Previous lecture slides showing style and format... 📄 Learning objectives for this unit... Then: Write a good lecture on AI coding assistants for CS3100'. Arrow points to OUTPUT: A rich, tailored lecture that references L4 specs, connects to the 6-step workflow, matches the course's pixel art style, uses SceneItAll examples. Labels indicate: 'References prior lectures', 'Matches course style', 'Uses course-specific examples'. The output GLOWS with quality. CENTER DIVIDER: Shows the 'context window' as a container—bigger context = better predictions. Size indicators: '4K tokens (early models)' → '100K tokens (modern)' → '1M+ tokens (frontier)'. KEY INSIGHT at bottom: 'The model is the same. The context is different. The output quality is DRAMATICALLY different. Your job is to provide the right context.' Style: Dramatic split comparison, gray/mediocre on top, golden/excellent on bottom. Clear cause-and-effect."
  alt="Split comparison: Top shows vague prompt 'Write lecture on AI assistants' producing generic output. Bottom shows same request with 40,000 words of course context producing tailored output with course-specific connections. Center shows context window sizes growing from 4K to 1M+ tokens. Key insight: Same model, different context, dramatically different quality."
/>
...
→ **Transition:** Let's look at the most common interaction mode—tab completion...
</aside>
...
## Tab Completion: Where You'll Spend 80% of Your Time

<Img
  src="/img/lectures/web/l13-tab-complete.webp"
  prompt="Concept: 'The Tab-Complete Experience Across Tools'. A side-by-side comparison showing tab completion in different AI coding tools. Show 4 code editor windows arranged in a 2x2 grid, each showing the SAME Java code being written with different tab-complete experiences: TOP-LEFT - GITHUB COPILOT: Code editor showing 'public void processOrder(Order o) {' with ghost text suggesting a full method implementation. Label shows 'Copilot' with characteristics: 'Mature, reliable, good context awareness'. Latency indicator: '~200ms'. Quality bar: HIGH. Unique feature callout: 'Sees your whole repo via GitHub'. TOP-RIGHT - CURSOR TAB: Similar code with ghost text, but showing MULTI-LINE suggestion with syntax highlighting in the ghost text. Label shows 'Cursor' with characteristics: 'Blazing fast, speculative edits'. Latency indicator: '~100ms'. Quality bar: HIGH. Unique feature callout: 'Predicts your NEXT edit, not just current line'. BOTTOM-LEFT - CODEIUM/WINDSURF: Code with ghost text. Label: 'Generous free tier, fast'. Latency: '~150ms'. Quality: GOOD. Callout: 'Best free option for heavy use'. BOTTOM-RIGHT - TABNINE: Code with shorter suggestion. Label: 'Local model option, privacy-focused'. Latency: '~50ms (local)'. Quality: MODERATE. Callout: 'Can run entirely offline'. CENTER COMPARISON: Key differentiators listed: 'Speed matters more than you think—200ms vs 100ms feels VERY different in flow state.' Below: 'All accept with Tab. The magic is what they suggest and how fast.' Style: Clean IDE screenshot mockups with clear labels, speed/quality indicators as visual meters."
  alt="2x2 grid comparing tab-complete across tools: Copilot (mature, ~200ms, sees GitHub repo), Cursor Tab (~100ms, predicts next edit), Codeium/Windsurf (free tier, ~150ms), Tabnine (local/offline, ~50ms). Center notes that speed differences feel significant in flow state."
/>

<aside className="notes">
**Tab completion is THE killer feature:**
- You'll use this 10x more than chat
- Speed matters enormously—100ms vs 200ms feels very different
- Quality of suggestions varies by context

**Tool comparison:**

**Copilot:**
- The baseline everyone compares to
- Good context awareness, reliable
- Deep GitHub integration (sees your whole repo)
- ~200ms latency typically

**Cursor Tab:**
- Noticeably faster (~100ms)
- "Speculative edits"—predicts your NEXT change
- Sometimes eerily prescient
- Why people pay $20/mo for Cursor

**Codeium/Windsurf:**
- Best free tier for heavy use
- Quality close to Copilot
- Good if you're cost-conscious

**Tabnine:**
- Can run LOCAL models (privacy!)
- Faster latency when local (~50ms)
- Lower quality but works offline

**The key insight:**
- Try different tools, feel the speed difference
- Tab-complete quality compounds over thousands of uses
- This is where "tool builder UX" really shows

→ **Transition:** Now let's look at the broader ecosystem...
</aside>
...
## Model Providers: Copilot and Claude Code

<div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1em', fontSize: '0.7em'}}>

<div style={{backgroundColor: 'rgba(0,120,212,0.1)', padding: '0.75em', borderRadius: '8px'}}>

**GitHub Copilot** (Microsoft/OpenAI)

- **Model:** GPT-4o, o1, o3-mini (OpenAI models)
...
- **Limitations:**
  - OpenAI models only
  - Less agentic capability

</div>

<div style={{backgroundColor: 'rgba(204,119,34,0.1)', padding: '0.75em', borderRadius: '8px'}}>

**Claude Code** (Anthropic)

- **Model:** Claude Sonnet 4, Claude Opus 4
- **Pricing:** Pay-per-use (API costs)
- **Integration:** Terminal-based, VS Code, JetBrains
- **Strengths:**
...
Both are building the models AND the tools—tight integration, but limited choice.
</p>

<aside className="notes">
**GitHub Copilot:**
- The original AI coding assistant (launched 2021)
- Microsoft owns GitHub AND has $13B invested in OpenAI
- Very mature—millions of users, lots of polish
- Best-in-class tab completion
- Free for students through GitHub Education!

**Claude Code:**
- Newer entrant from Anthropic
- Claude models known for strong reasoning
- More "agentic"—can execute multi-step plans
- Better at understanding large codebases
- Terminal-first design (CLI tool)

**The tradeoff:**
- Copilot: Proven, polished, predictable
- Claude Code: Cutting-edge capabilities, less mature

**For this course:** We use Copilot because you all have free access.

→ **Transition:** Now let's look at the tool builders...
</aside>
...
## Tool Builders: Cursor, Windsurf, and Others

<Img
  src="/img/lectures/web/l13-tool-builders.webp"
  prompt="Concept: 'The Tool Builder Innovation Layer'. A showcase of tool-builder IDEs with their unique features. Show 3-4 tool screenshots or stylized representations arranged in a grid: CURSOR (top-left): A VS Code-like editor with highlighted features - 'Composer' multi-file editing panel, 'Codebase indexing' indicator, model selector showing 'Claude / GPT-4 / Gemini'. Tagline: 'Fork of VS Code with AI superpowers'. WINDSURF (top-right): Similar editor interface with 'Cascade' agentic flow visualization, showing AI executing multiple steps autonomously. Tagline: 'Agentic-first IDE from Codeium'. CLINE (bottom-left): VS Code extension view showing autonomous coding agent with approval workflow. Tagline: 'Open-source agentic assistant'. CONTINUE (bottom-right): IDE showing model configuration panel with local model options (Ollama, LM Studio). Tagline: 'Open-source, bring your own model'. CENTER CALLOUT: 'What they share: Model flexibility, rapid feature iteration, API-based pricing'. Bottom annotation: 'Tool builders move FAST—new features weekly. Model providers move slower but more reliably.' Style: Clean product showcase with subtle screenshots or wireframes, modern tech aesthetic."
  alt="Grid showcasing tool-builder IDEs: Cursor with multi-file Composer and model selection, Windsurf with agentic Cascade workflows, Cline as open-source VS Code extension, Continue with local model support. Center callout emphasizes shared traits: model flexibility and rapid iteration."
/>

<aside className="notes">
**The tool builder landscape:**

**Cursor:**
- Fork of VS Code with AI built-in
- "Composer" for multi-file edits
- Can use Claude, GPT-4, or other models
- ~$20/mo + API costs for heavy use
- Currently the most popular "power user" choice

**Windsurf (from Codeium):**
...
- Very agentic—can run commands, edit files
- You provide your own API keys
- Great for learning how these tools work

**Continue:**
- Open-source
- Works with local models (Ollama, LM Studio)
- No cloud required—privacy-focused

**Why they move fast:**
- Smaller teams, focused mission
- Not building models—just UX
- Can adopt ANY new model immediately
...
## Choosing Your Tools: Tradeoffs to Consider

<div style={{fontSize: '0.65em'}}>

| Factor | Model Providers (Copilot, Claude Code) | Tool Builders (Cursor, Windsurf) |
|--------|----------------------------------------|----------------------------------|
| **Cost model** | Flat subscription (~$10-20/mo) | Subscription + API usage (variable) |
| **Model choice** | Locked to provider's models | Mix and match (Claude, GPT, Gemini, local) |
| **Feature pace** | Slower, more stable releases | Rapid iteration, frequent updates |
| **Agentic capabilities** | Growing (Copilot Workspace, Claude Code) | Often more advanced |
| **Risk** | Provider deprecates features | Startup may pivot or fail |
| **Enterprise readiness** | Generally more mature | Varies widely |

</div>

<p style={{fontSize: '0.85em', marginTop: '0.75em'}}>
<strong>Recommendation:</strong> Start with Copilot (free for students), explore others as you develop preferences.
</p>

<aside className="notes">
**How to think about this:**

**Choose Model Providers when:**
- You want predictable costs
- You're in an enterprise environment
- Stability matters more than cutting-edge features
- You're just getting started (Copilot is free!)

**Choose Tool Builders when:**
- You want the latest capabilities
- You need specific model characteristics
- You're willing to manage API costs
- You want to experiment with different models

**The "agentic" trend:**
- Both categories are racing toward more autonomous AI
- Copilot Workspace, Claude Code, Cursor Composer, Windsurf Cascade
- AI that can execute multi-step plans, not just suggest code

**For this course:**
- Use Copilot—it's free and solid
- If you want to explore, Cursor's free tier is good
- The SKILLS transfer between tools

→ **Transition:** Now let's look at what all these tools are actually good at...
</aside>
...
## The Agentic Future: AI That Takes Actions

<Img
  src="/img/lectures/web/l13-agentic.webp"
  prompt="Concept: 'The Spectrum of AI Assistance - From Autocomplete to Autonomous Agent'. A horizontal spectrum/timeline showing the evolution of AI coding assistance, with current tools mapped to their position. LEFT END - 'AUTOCOMPLETE' (simplest): Icon of cursor with single-line ghost text. Label: 'Predict the next few tokens'. Examples: Early Copilot, Tabnine. User control: HIGH. AI autonomy: LOW. MIDDLE-LEFT - 'CHAT + APPLY': Icon of chat bubble with code diff. Label: 'Generate code blocks, user applies'. Examples: ChatGPT, Claude.ai for coding. User control: HIGH. AI autonomy: LOW-MEDIUM. MIDDLE - 'INTEGRATED EDIT': Icon of IDE with AI sidebar making multi-line edits. Label: 'AI edits files directly with approval'. Examples: Copilot Chat, Cursor Tab. User control: MEDIUM. AI autonomy: MEDIUM. MIDDLE-RIGHT - 'COMPOSER/MULTI-FILE': Icon showing multiple files being edited simultaneously. Label: 'Coordinate changes across codebase'. Examples: Cursor Composer, Claude Code. User control: MEDIUM. AI autonomy: MEDIUM-HIGH. RIGHT END - 'AUTONOMOUS AGENT' (most advanced): Icon of AI with multiple tool connections (terminal, browser, file system). Label: 'Plan and execute multi-step tasks'. Examples: Claude Code agentic mode, Devin (demo), Cursor Agent. User control: LOW (approval checkpoints). AI autonomy: HIGH. ANNOTATIONS: Arrow showing 'Industry direction →' pointing right. Warning callout near autonomous end: 'More power = more risk. Always review!' Below spectrum: 'Your expertise determines how much autonomy is safe. The 6-step workflow applies at EVERY level.' Style: Clean horizontal spectrum with gradient from blue (user control) to orange (AI autonomy), clear iconography for each level."
  alt="Horizontal spectrum from Autocomplete (left, high user control) through Chat+Apply, Integrated Edit, Multi-file Composer, to Autonomous Agent (right, high AI autonomy). Current tools mapped to positions. Arrow shows industry trending toward more autonomy, with warning that more power requires more careful review."
/>

<p style={{fontSize: '0.8em', marginTop: '0.5em'}}>
The industry is moving right. Your evaluation skills become MORE important, not less.
</p>
...
**Autocomplete (left):**
- AI predicts next few tokens
- You accept or reject immediately
- Very safe—small changes, easy to evaluate

**Chat + Apply:**
- AI generates larger blocks
- You copy/paste or click "apply"
- Still high control

**Integrated Edit:**
- AI can modify your files directly
- But you approve each change
- Copilot Chat, basic Cursor

**Multi-file Composer:**
- AI coordinates changes across many files
- Refactoring, feature implementation
- Harder to review—more trust required

**Autonomous Agent:**
- AI plans and executes multi-step tasks
- Can run commands, browse docs, iterate
- Maximum power, maximum risk

**The key insight:**
- Industry is moving toward MORE autonomy
- But YOUR skills become MORE important
- You need to evaluate larger chunks of work
- The 6-step workflow scales to all levels

**Connection to task familiarity:**
- More autonomy = need MORE expertise to evaluate
- Don't use agentic mode for unfamiliar domains!

→ **Transition:** Now let's look at what these tools are actually good at...
</aside>
