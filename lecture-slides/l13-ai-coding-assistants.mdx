...
<Img
  ...
  prompt="Concept: 'The AI Coding Tool Ecosystem - Model Providers vs Tool Builders'. A split infographic showing two distinct categories of AI coding tools, styled as two different storefronts or factories. LEFT SIDE - 'MODEL PROVIDERS' (labeled 'The Source'): A large factory/lab building with logos for Microsoft/OpenAI (powering GitHub Copilot) and Anthropic (powering Claude Code). Inside the factory, show LLM 'brains' being created - neural networks, training data, compute clusters. Output conveyor belt shows: 'Their Own IDE Tools' - Copilot icon going to VS Code, Claude Code icon going to terminal/IDE. Key characteristics listed: 'Control the models', 'Flat subscription pricing', 'Optimized integration', 'Limited model choice'. A price tag shows '$10-20/month flat'. RIGHT SIDE - 'TOOL BUILDERS' (labeled 'The Craftsmen'): A workshop/studio building with logos for Cursor, Windsurf, Cline, Continue. These companies are shown PURCHASING API access from the model providers (dollar signs flowing left). Inside their workshop, they're building sophisticated IDE features - multi-file editing, codebase indexing, agentic workflows. Key characteristics: 'Best-in-class UX', 'Model flexibility (use any model)', 'Pay-per-use API costs', 'Rapid innovation'. A meter shows 'API costs can vary'. CENTER BRIDGE: Arrows showing the relationship - Model providers SELL API access, Tool builders BUY it and add value. Annotation: 'Tool builders compete on UX and features. Model providers compete on model quality.' BOTTOM: A spectrum showing the tradeoff - 'Predictable cost ←→ Maximum flexibility'. Style: Clean, modern business/tech infographic with distinct color coding for each category (blue for providers, orange for builders)."
...
- They DON'T make models—they buy API access
...
- Cursor with @codebase often just... figures it out
...
- `@codebase` searches everything
- `@file` pulls in specific files
- Tool indexes and retrieves automatically
...
| **Politeness affects performance** | "Please" and "Thank you" | Models don't have feelings. Be clear, not polite. (Though it doesn't hurt!) |
...
- The model isn't offended by terse prompts
...
→ **Transition:** But you still need to evaluate—let's talk about that...
</aside>
...
## Sidebar: What Can Tools Actually Find?

<Img
  src="/img/lectures/web/l13-context-frontier.webp"
  prompt="Concept: 'The Context Discovery Frontier'. Three concentric rings diagram showing where context lives and whether AI tools can find it. INNER RING (solid green, labeled 'Your Codebase'): Icons for code files, type definitions, test files, call graphs. Fully accessible. Label: 'Tools explore this fully'. MIDDLE RING (dashed yellow border, labeled 'Connected Systems'): Icons for issue tracker tickets, team wiki pages, chat threads, build/CI logs. Arrows showing the boundary EXPANDING outward. Label: 'The expanding frontier - standardized connectors (MCP) are opening this up'. Some connectors shown as plugs linking to the inner ring. OUTER RING (red, labeled 'Human Knowledge'): Icons for thought bubbles, meeting notes, rejected ideas crossed out, verbal conversations. Label: 'Only findable if you document it'. A KEY INSIGHT arrow from outer ring pointing inward with text: 'Document it → it moves inward'. Shows a design doc being written and then appearing in the middle ring. BOTTOM annotation: 'The more you write down, the more tools can discover.' Style: Clean concentric ring diagram, traffic light colors (green inner, yellow middle, red outer), clear iconography, expanding/dynamic feel for the middle ring."
  alt="Three concentric rings showing context discovery: Inner green ring (codebase) is fully searchable. Middle yellow ring (connected systems like issue trackers, wikis) is the expanding frontier as tools gain standardized connectors. Outer red ring (human knowledge) requires documentation to become findable. Arrow shows: document it and it moves inward."
/>

<p style={{fontSize: '0.8em', marginTop: '0.5em', color: '#9370DB'}}>
<strong>Key insight:</strong> The more you document, the more tools can find. Design docs turn human knowledge into discoverable context.
</p>

<aside className="notes">
**The three layers of context:**

**1. Your Codebase (inner ring - tools find this easily):**
- Static analysis indexes everything
- Types, call graphs, test files, usage patterns
- This is what we talked about earlier with Cursor's @codebase
- Already solved—tools are great at this

**2. Connected Systems (middle ring - the expanding frontier):**
- Issue trackers, team wikis, chat history, CI/CD logs
- Historically: you had to copy-paste from these into your prompt
- What's changing: standardized protocols (like MCP) let tools connect directly
- Your AI assistant can now: read the issue ticket, check why the build failed, pull relevant wiki pages
- This boundary is moving outward rapidly

**3. Human Knowledge (outer ring - still requires you):**
- Why you rejected alternative X
- What the stakeholder said in that meeting
- The security concern your teammate mentioned at lunch
- Tacit knowledge that only exists in your head
- Tools can't find what was never written down

**The key insight for students:**
- Documentation isn't just for humans anymore
- It's how you make context FINDABLE for tools
- A design decision you don't document is invisible to AI
- This connects to why we'll spend time on Architecture Decision Records later

**Forward reference to architecture lectures:**
- "We'll talk more about this when we cover software architecture"
- ADRs (Architecture Decision Records) capture the WHY
- Design docs capture rejected alternatives
- These aren't bureaucracy—they're context infrastructure

**Connection to earlier slides:**
- "Modern Tools Change the Game" said tools can't find requirements not in code
- This slide explains WHERE that context lives and how to make it findable
